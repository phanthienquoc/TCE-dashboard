...
const emptyBinance = { apiKey: '', apiSecret: '' };
...
export default function PlatformsPanel() {
  ...
  useEffect(() => {
    let active = true;
    setCurrent(null); setTransactionId(''); setStatus('');
  ...
  useEffect(() => {
    let active = true;
    hasBinanceCredentials(binanceEnvironment).then((v) => active && setBinanceSaved(v)).catch(() => active && setBinanceSaved(false));
    return () => { active = false; };
  }, [binanceEnvironment]);
  ...
  const button = (label, action, primary = false, name = '') => (
    <button type="button" className={`${styles.button} ${primary ? styles.primary : ''}`} onClick={action} disabled={busy}>
      {busy && busyAction === name ? 'Working…' : label}
    </button>
  );
  const run = async (name, fn, done) => {
    setBusy(true); setBusyAction(name); setStatus('Working…');
    try { done(await fn()); } catch (e) { setStatus(message(e, 'Request failed.')); }
    finally { setBusy(false); setBusyAction(''); }
  };
  const setS = (k, v) => setSsi((x) => ({ ...x, [k]: v }));
  const setB = (k, v) => setBinance((x) => ({ ...x, [k]: v }));
  const setO = (k, v) => setOrder((x) => ({ ...x, [k]: v }));

  const saveSsi = () => run('ssi-save', async () => {
    if (!ssi.apiKey || !ssi.apiSecret || !ssi.accountNo) throw new Error('SSI API Key, API Secret and Account No. are required.');
    await saveSsiCredentials(environment, ssi); return true;
  }, () => { setSaved(true); setStatus('SSI credentials saved.'); });

  const saveB = () => run('binance-save', async () => {
  ...
  const testSsi = () => run('ssi-test', () => testSsiConnection({ ...authBody(), credentials: ssi }), (r) =>
    setStatus(`SSI connected • API v${r?.apiVersion || '3'} • ${(r?.accounts || []).length} account(s). Order stream starts automatically.`));
  const requestOtp = () => run('otp', async () => {
    const r = await requestSsiOtp({ environment, credentials: ssi });
    const id = r?.data?.transactionId || r?.transactionId || '';
    if (id) setTransactionId(id); return id;
  }, (id) => setStatus(id ? `SSI approval requested • ${id}` : 'SSI OTP / approval requested.'));
  const loadCurrent = () => run('current', () => getCurrentSsiInfo(authBody()), (r) => { setCurrent(r); setStatus('SSI current info loaded.'); });
  const sync = () => run('sync', () => syncSsiPortfolio(authBody()), (r) =>
    setStatus(`Synced • ${r?.positionsSynced || 0} positions • ${r?.ordersSynced || 0} orders. Order stream active.`));
  ...
  const sendOrder = () => run('order', () => {
    if (!confirmReal) throw new Error(`Confirm the ${binanceEnvironment === 'testnet' ? 'Testnet' : 'real'} order checkbox first.`);
    if (!order.quantity || Number(order.quantity) <= 0) throw new Error('Quantity must be greater than 0.');
    if (order.type === 'LIMIT' && !order.price) throw new Error('Limit price is required.');
    if ((order.type === 'STOP' || order.type === 'STOP_MARKET') && !order.triggerPrice) throw new Error('Trigger price is required.');
    return placeBinanceOrder(payload());
  }, (r) => { setOrderResult(r); setStatus(`Order ${r?.status || 'submitted'} • ${r?.orderId || ''}`); });

  const exitPayload = (triggerPrice) => ({
    environment: binanceEnvironment,
    symbol: order.symbol,
  ...
  const sendTp = () => run('tp', () => {
    if (!confirmReal) throw new Error(`Confirm the ${binanceEnvironment === 'testnet' ? 'Testnet' : 'real'} order checkbox first.`);
    if (!tp) throw new Error('TP trigger price is required.');
    return placeBinanceTp(exitPayload(tp));
  }, (r) => { setOrderResult(r); setStatus(`TP ${r?.status || 'submitted'} • ${r?.orderId || ''}`); });
  const sendSl = () => run('sl', () => {
    if (!confirmReal) throw new Error(`Confirm the ${binanceEnvironment === 'testnet' ? 'Testnet' : 'real'} order checkbox first.`);
    if (!sl) throw new Error('SL trigger price is required.');
    return placeBinanceSl(exitPayload(sl));
  }, (r) => { setOrderResult(r); setStatus(`SL ${r?.status || 'submitted'} • ${r?.orderId || ''}`); });

  const isTestnet = binanceEnvironment === 'testnet';
  const envLabel = isTestnet ? 'Testnet' : 'Production';

  return <>
    <section className={`section page-section ${styles.page}`}>
      <div className={styles.head}><div><span className="eyebrow">PLATFORM CONFIG</span><h2>SSI FastConnect</h2><small>Credentials stay server-side and are never returned.</small></div><span className={`${styles.status} ${saved ? styles.statusOk : ''}`}>{saved ? '● CONFIGURED' : '○ NOT CONFIGURED'}</span></div>
      <div className={styles.fields}>
        ...
        <label className={styles.field}>Client ID<input className={styles.input} value={ssi.clientId} onChange={e => setS('clientId', e.target.value)} /></label>
        <label className={styles.field}>API Key<input className={styles.input} type="password" value={ssi.apiKey} onChange={e => setS('apiKey', e.target.value)} /></label>
        <label className={styles.field}>API Secret<input className={styles.input} type="password" value={ssi.apiSecret} onChange={e => setS('apiSecret', e.target.value)} /></label>
        <label className={styles.field}>Account No.<input className={styles.input} value={ssi.accountNo} onChange={e => setS('accountNo', e.target.value)} /></label>
        <label className={styles.field}>OTP<input className={styles.input} type="password" value={otp} onChange={e => setOtp(e.target.value)} /></label>
      </div>
      <label className={styles.field}>Private Key<textarea className={styles.textarea} value={ssi.privateKey} onChange={e => setS('privateKey', e.target.value)} rows={4} /></label>
      <div className={styles.actions}>{button('Save SSI', saveSsi, true, 'ssi-save')}{button('Test connection', testSsi, false, 'ssi-test')}{button('Request OTP / Approval', requestOtp, false, 'otp')}{button('Get current info', loadCurrent, false, 'current')}{button('Sync portfolio', sync, false, 'sync')}</div>
      <div className={styles.note}><b>Auth state</b><span>{transactionId ? `Transaction: ${transactionId} • ` : ''}{status || 'Ready.'}</span></div>
      {current && <pre className={styles.output}>{JSON.stringify(current, null, 2)}</pre>}
    </section>
