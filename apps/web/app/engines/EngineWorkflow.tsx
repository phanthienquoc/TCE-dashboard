    ...
    <div className={`w-[230px] rounded-2xl border p-3 shadow-xl backdrop-blur ${statusClass}`}>
      ...
      <div className="flex items-start gap-3">
        <button
          ...
          className={`nodrag mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-xl border transition ${
            data.enabled
        ...
        <Link href={data.href ?? '#'} onClick={event => event.stopPropagation()} className="nodrag min-w-0 flex-1" aria-label={`Open ${data.name}`}>
        ...
        <Link href={data.href ?? '#'} onClick={event => event.stopPropagation()} className="nodrag text-slate-400 hover:text-white" aria-label={`Configure ${data.name}`}>
...
export function EngineWorkflow({ engines }: { engines: WorkflowEngine[] }) {
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="px-1 text-[11px] text-slate-500">Kéo node để sắp xếp flow. Dependency/edge vẫn do Runtime backend quyết định.</p>
    </section>
  );
}
