    ...
    <div className={`w-[230px] rounded-2xl border p-3 shadow-xl backdrop-blur ${statusClass}`}>
      ...
      <div className="flex items-start gap-3">
        <button
          ...
          className={`nodrag mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-xl border ${data.enabled ? 'border-emerald-400/30 bg-emerald-400/15 text-emerald-300' : 'border-white/10 bg-white/5 text-slate-500'}`}
        ...
        >
          <div className="flex items-center gap-2">
            <span
              className={`size-2 rounded-full ${data.status === 'ACTIVE' ? 'bg-emerald-400' : data.status === 'ERROR' ? 'bg-rose-400' : 'bg-slate-500'}`}
            />
        ...
        <Link
          href={data.href ?? '#'}
          onClick={event => event.stopPropagation()}
          className="nodrag text-slate-400 hover:text-white"
      ...
      <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-2 text-[10px]">
        ...
        <span className="max-w-[130px] truncate text-slate-500">
          {data.dependencies.length
            ? `Depends on ${data.dependencies.join(', ')}`
            : 'No dependency'}
        </span>
      </div>
      <Handle
...
export function EngineWorkflow({ engines }: { engines: WorkflowEngine[] }) {
  const initialNodes = useMemo<Node<WorkflowEngine, 'engine'>[]>(() => {
    const layout: Record<string, { x: number; y: number }> = {
      'tce-decision': { x: 70, y: 100 },
      'ssi-execution': { x: 70, y: 420 },
      'binance-market': { x: 360, y: 420 },
      'binance-xau': { x: 360, y: 740 },
    };
    return engines.map(engine => ({
      id: engine.id,
      type: 'engine',
      position: layout[engine.id] ?? { x: 70, y: 100 + engines.indexOf(engine) * 320 },
      data: engine,
      draggable: true,
    }));
  }, [engines]);
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="px-1 text-[11px] text-slate-500">
        Kéo node để sắp xếp flow. Dependency/edge do Runtime backend quyết định.
      </p>
    </section>
  );
}
