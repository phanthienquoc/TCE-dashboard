...
const navigation = [
  { id: 'overview', label: 'Overview', icon: Home }, { id: 'positions', label: 'Positions', icon: BarChart3 },
  { id: 'orders', label: 'Orders', icon: ArrowLeftRight }, { id: 'engine', label: 'Engine', icon: Cpu },
...
export default function EnginesPage() {
  ...
  const select = (id: string) => { if (id === 'engine') return; if (id === 'notifications') return window.location.assign('/notifications'); if (id === 'settings') return window.location.assign('/dashboard?tab=settings'); if (id === 'overview') return window.location.assign('/dashboard'); return window.location.assign(`/dashboard?tab=${id}`); };
  return <main className="app-shell"><header className="app-header"><div className="app-container app-header-inner"><div className="flex items-center gap-3"><Link href="/dashboard" className="touch-target grid place-items-center rounded-2xl border border-violet-200/[0.09] bg-white/[0.02] text-[#a88bb5]" aria-label="Back to dashboard"><ArrowLeft className="size-4" /></Link><div><p className="eyebrow">TCE</p><p className="account-email">Engine runtime</p></div></div></div></header><div className="app-container app-content engine-page-content"><section className="page-heading"><div><p className="eyebrow">Runtime</p><h1>Engine</h1><p className="page-subtitle">Control and inspect TCE trading engines.</p></div></section><EngineControlPanel /></div><NavigationDock items={navigation.map(item => ({ ...item, active: item.id === 'engine' }))} onSelect={select} /></main>;
}
