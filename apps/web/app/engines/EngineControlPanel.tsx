...
export default function EngineControlPanel() {
  ...
  useEffect(() => {
    ...
    const apply = (rows: EngineState[]) => {
      ...
      setEnabled(next); setLoading(false);
    };
    if (cachedEngines !== null) { apply(cachedEngines as EngineState[]); return () => { mounted = false; }; }
  ...
  async function toggle(id: EngineId) {
    ...
    const nextEnabled = !enabled[id]; const previous = enabled[id];
    setEnabled(current => ({ ...current, [id]: nextEnabled })); setUpdating(id);
  ...
  return <section className="engine-control-section">
    <div className="page-heading engine-control-heading"><div className="min-w-0"><p className="eyebrow">Runtime</p><h1>Engine Control</h1><p className="page-subtitle">Engine state is loaded from the backend account configuration.</p></div><div className="hero-status">{activeCount} ACTIVE</div></div>
    <Card className="panel-card engine-list-card"><div className="engine-list">{engines.slice(0, 5).map(engine => <div key={engine.id} className="engine-row">
      ...
      <Link href={engine.id === 'binance-xau' ? '/xau' : `/engines/${engine.id}`} className="engine-copy"><div className="engine-title-row"><p>{engine.name}</p><span>{engine.category}</span></div><p className="engine-description">{engine.description}</p></Link>
    ...
    </div>)}</div>{engines.length > 5 && <div className="list-footer"><span>Showing 5 of {engines.length}</span><Link href="/engines">View all <ChevronRight className="size-3" /></Link></div>}</Card>
