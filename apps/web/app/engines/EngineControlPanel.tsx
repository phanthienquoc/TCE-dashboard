...
export default function EngineControlPanel() {
  ...
  useEffect(() => {
    let mounted = true;
    void dashboardApi.engines().then((response) => {
      if (!mounted) return;
      const next = { 'tce-decision': true, 'ssi-execution': true, 'binance-market': true } as EnabledState;
      for (const row of response.data ?? []) {
        if (row?.engineId in next) next[row.engineId as EngineId] = String(row.status).toUpperCase() === 'ACTIVE';
      }
      setEnabled(next);
  ...
  return (
    <section className="space-y-4 pb-4">
      <div className="page-heading !mb-2">
        <div><p className="eyebrow">Runtime</p><h1>Engine Control</h1><p className="page-subtitle">Tap the power icon to change engine status.</p></div>
      ...
      <div className="flex items-start gap-2 px-1 text-xs leading-5 text-[#75697d]"><Settings2 className="mt-0.5 size-3.5 shrink-0" /> Engine status is persisted per account on the backend.</div>
    </section>
  );
}
