...
export default function EngineControlPanel() {
  ...
  async function toggle(id: EngineId) {
    ...
    if (id === 'binance-xau') { window.location.href = '/engines/xau'; return; }
  ...
  return (
    <section className="engine-control-section">
      ...
      <Card className="panel-card engine-list-card">
        <div className="engine-list">
          {engines.slice(0, 5).map(engine => (
            <div key={engine.id} className="engine-row">
              ...
              <Link href={engine.id === 'binance-xau' ? '/engines/xau' : `/engines/${engine.id}`} className="engine-copy">
        ...
        {engines.length > 5 && <div className="list-footer"><span>Showing 5 of {engines.length}</span><Link href="/engines">View all <ChevronRight className="size-3" /></Link></div>}
      </Card>
