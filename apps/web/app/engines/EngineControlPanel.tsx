      ...
      <Card className="panel-card engine-list-card">
          ...
          {engines.slice(0, 5).map(engine => (
            <div key={engine.id} className="engine-row">
              <button
                ...
                className={`engine-power ${enabled[engine.id] ? 'is-active' : ''} disabled:opacity-50`}
              >
                <Power className="size-4" />
              </button>
              <Link
                href={engine.id === 'binance-xau' ? '/xau' : `/engines/${engine.id}`}
              ...
              <div className="engine-meta">
                <span className={enabled[engine.id] ? 'is-active-text' : ''}>
        ...
        {engines.length > 5 && (
          <div className="list-footer">
            <span>Showing 5 of {engines.length}</span>
            <Link href="/engines">
