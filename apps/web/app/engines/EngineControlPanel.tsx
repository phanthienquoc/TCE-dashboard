  ...
  return (
    <section className="space-y-4 pb-4">
          ...
          {engines.map(engine => (
            <div key={engine.id} className="flex min-h-[82px] items-center gap-3 px-4 py-3 sm:px-5">
              ...
              <Link href={`/engines/${engine.id}`} className="min-w-0 flex-1 py-1">
                ...
                <p className="mt-1 line-clamp-2 text-xs leading-5 text-[#75697d]">
                  {engine.description}
                </p>
              </Link>
              <span
                className={`shrink-0 text-[9px] font-semibold uppercase tracking-[.12em] ${enabled[engine.id] ? 'text-emerald-300' : 'text-[#66596e]'}`}
              >
              ...
              <Link
                ...
                className="grid size-9 shrink-0 place-items-center rounded-lg text-[#75697d] hover:bg-white/[0.04] hover:text-white"
              >
      ...
      <div className="flex items-start gap-2 px-1 text-xs leading-5 text-[#75697d]">
        <Settings2 className="mt-0.5 size-3.5 shrink-0" /> Engine state and strategy configuration
        are persisted per account on the backend.
      </div>
    </section>
  );
}
