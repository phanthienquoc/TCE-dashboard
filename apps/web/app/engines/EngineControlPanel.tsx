...
export default function EngineControlPanel() {
  ...
  useEffect(() => {
    ...
    Promise.all([dashboardApi.engines(), dashboardApi.engineConfig()]).then(([stateResponse, configResponse]) => {
      ...
      const merged = ENGINE_REGISTRY.map(definition => ({
        ...
        enabled: byId.get(definition.id)?.enabled ?? String(stateById.get(definition.id)?.status).toUpperCase() === 'ACTIVE',
  ...
  async function toggle(id: EngineId) {
    ...
    try {
      ...
      setConfigs(current => current.map(row => row.engineId === id ? { ...row, enabled: nextEnabled } : row));
    } catch {
    ...
    <section className="engine-control-section">
      <div className="page-heading engine-control-heading">
        <div className="min-w-0">
          ...
          <p className="page-subtitle">Engine state and configuration are loaded from the backend account configuration.</p>
        </div>
          ...
          {configs.map(engine => {
            ...
            return (
              <div key={engine.engineId} className="engine-row">
                ...
                <Link href={engine.engineId === 'binance-xau' ? '/xau' : `/engines/${engine.engineId}`} className="engine-copy">
                  <div className="engine-title-row"><p>{definition.name}</p><span>{definition.category}</span></div>
                  <p className="engine-description">{definition.description}</p>
                </Link>
                <div className="engine-meta">
                  <span className={engine.enabled ? 'is-active-text' : ''}>{engine.enabled ? 'ACTIVE' : 'INACTIVE'}</span>
      ...
      <div className="engine-note"><Settings2 className="size-3.5 shrink-0" />Configuration is persisted per account; registry values are fallback defaults only.</div>
    </section>
  );
}
