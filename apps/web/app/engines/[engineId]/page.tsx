    ...
    <div className="engine-detail-page">
      <div className="page-heading">
        <div className="min-w-0">
          ...
          <p className="eyebrow">{engine.platform} · {engine.category}</p>
          <h1>{engine.name}</h1>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted">
                    Sync all SSI sub-accounts, including Cash and Margin holdings, into the TCE portfolio.
                  </p>
                </div>
                <Button className="touch-target shrink-0" disabled={syncingPortfolio} onClick={() => void syncPortfolio()}>
                  <RefreshCw className={`size-4 ${syncingPortfolio ? 'animate-spin' : ''}`} />{' '}
                  {syncingPortfolio ? 'Syncing…' : 'Sync portfolio'}
                </Button>
              </div>
              {portfolioResult && (
                <div className={`rounded-xl border px-3 py-2.5 text-xs ${portfolioResult.ok ? 'border-emerald-300/15 bg-emerald-300/[0.05] text-emerald-200' : 'border-red-300/15 bg-red-300/[0.05] text-red-200'}`}>
            ...
            <CardContent className="space-y-3 p-5 sm:p-6">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted">
                    Trigger the SSI market-price sync manually without waiting for the hourly scheduler.
                  </p>
                </div>
                <Button className="touch-target shrink-0" disabled={syncing} onClick={() => void syncMarketData()}>
                  <RefreshCw className={`size-4 ${syncing ? 'animate-spin' : ''}`} />{' '}
                  {syncing ? 'Syncing…' : 'Sync market data'}
                </Button>
              </div>
              {syncResult && (
                <div className={`rounded-xl border px-3 py-2.5 text-xs ${syncResult.ok ? 'border-emerald-300/15 bg-emerald-300/[0.05] text-emerald-200' : 'border-red-300/15 bg-red-300/[0.05] text-red-200'}`}>
                  {syncResult.message}
                </div>
              )}
            </CardContent>
      ...
      <Card className="panel-card">
        <CardContent className="space-y-5 p-5 sm:p-6">
          ...
          <div className="flex items-center justify-between gap-3 border-t border-violet-200/[0.07] pt-5">
            ...
            <Button className="touch-target" disabled={loading || saved} onClick={() => void save()}>
