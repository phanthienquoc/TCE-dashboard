...
export default function DashboardPage() {
  ...
  return <main>
    ...
    {tab === 'overview' && <><section className="hero"><div><span>Account portfolio</span><strong>{money(deployed)}</strong><small>Your positions • P/L {money(account.unrealized_pnl)}</small></div><div className="ring" style={{ '--p': `${recoveryPct}%` }}><b>{recoveryPct}%</b></div></section><section className="grid"><article><span>Đang triển khai</span><b>{money(deployed)}</b></article><article><span>Unrealized P/L</span><b>{money(account.unrealized_pnl)}</b></article><article><span>Open positions</span><b>{positions.length}</b></article><article><span>Max positions</span><b>{account.max_positions || 2}</b></article></section><section className="section"><div className="section-head"><h2>Shared pools</h2><span>{(data?.pools || []).length}</span></div><div className="quick-links">{(data?.pools || []).slice(0, 4).map((pool) => <div className="quick-card" key={pool.symbol}><b>{pool.symbol}</b><span>{pool.status || 'ACTIVE'}</span></div>)}</div></section><section className="section"><div className="section-head"><h2>Next positions</h2><span>Shared</span></div>{candidates.length ? candidates.slice(0, 5).map((candidate) => <div className="planned-row" key={`${candidate.rank}-${candidate.symbol}`}><span className="slot">{candidate.rank}</span><div><b>{candidate.symbol}</b><small>{candidate.target_quantity ? `${num(candidate.target_quantity)} cp` : ''}{candidate.target_price ? ` • target ${num(candidate.target_price)}` : ''}</small></div><span className="planned-status">{candidate.status}</span></div>) : <div className="empty">No shared candidates</div>}</section></>}
