...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!active) return;
        if (err.message === 'Session expired' || err.response?.status === 401) { clearSession(); router.replace('/login'); return; }
  ...
  return <main>
    ...
    <header className="app-header"><div><span className="eyebrow">TCE • TREASURY CASH EXTRACTION</span><h1>{title}</h1></div><button className="live" onClick={signOut}>Sign out</button></header>
