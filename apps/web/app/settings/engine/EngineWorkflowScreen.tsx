...
const DEPENDENCIES: Record<string, string[]> = { 'tce-decision': [], 'ssi-execution': ['tce-decision'], 'binance-market': [], 'binance-xau': ['binance-market'] };
...
export default function EngineWorkflowScreen() {
  ...
  async function load() {
    ...
    try {
      ...
      setRuntime(ENGINE_REGISTRY.map(definition => { const row = rows.find(item => item.engineId === definition.id); return { id: definition.id, name: definition.name, shortName: definition.id === 'ssi-execution' ? 'SSI Execution' : definition.id === 'binance-market' ? 'Binance Market' : definition.id === 'binance-xau' ? 'Binance XAU' : 'TCE Decision', category: definition.category, description: definition.description, status: row?.status ?? 'PAUSED', enabled: Boolean(row?.configuredEnabled), dependencies: row?.dependencies ?? DEPENDENCIES[definition.id] ?? [], configSummary: definition.platform, href: definition.id === 'binance-xau' ? '/xau' : `/settings/engine/${definition.id}`, onToggle: toggle } satisfies WorkflowEngine; }));
