            ...
            <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.035]">
              {ENGINE_REGISTRY.map(engine => (
                <Link
                  ...
                  className="flex min-h-[76px] items-center gap-4 border-b border-white/10 px-4 last:border-b-0 active:bg-white/5"
                >
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-white/5 text-sky-300">
                    <Cpu className="size-5" />
                  </span>
                  <span className="min-w-0 flex-1">
                    <strong className="block text-[16px] font-medium text-white">{engine.name}</strong>
                    <span className="block truncate text-sm text-slate-400">{engine.category} · {engine.platform}</span>
                  </span>
