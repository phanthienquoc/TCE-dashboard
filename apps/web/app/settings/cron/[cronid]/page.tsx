    ...
    <DashboardShell view="settings">
      {() => (
        <main className="tce-settings-card">
          ...
          <div className="tce-section-row">
            <small>Default: OFF · 10% · 60 minutes · VN market session only</small>
          ...
          {config.lastRunAt && (
            <small>Last run: {new Date(config.lastRunAt).toLocaleString()}</small>
          )}
        </main>
      )}
    </DashboardShell>
