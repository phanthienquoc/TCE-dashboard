...
export default function SSIPlatform({ onMessage }: Props) {
  ...
  const requestOtp = async () => {
    if (!credentials.clientId.trim() || !credentials.apiKey.trim() || !credentials.apiSecret.trim() || !credentials.privateKey.trim()) {
      setResult({ ok: false, message: 'Client ID, API Key, API Secret and Private Key are required before requesting SSI approval.' });
      return false;
    }
    setBusy(true);
    setResult(null);
  ...
  return (
    <section className="mb-5 overflow-hidden rounded-[22px] border border-violet-200/[0.09] bg-[#150d1d] shadow-[0_18px_50px_rgba(0,0,0,.18)]">
      ...
      {open && <div className="border-t border-violet-200/[0.07] px-5 pb-5 pt-4">
        <div className="mb-5 grid grid-cols-2 gap-2"><StepIndicator number="1" title="Credentials" active={authStep === 'credentials'} done={authStep !== 'credentials'} /><StepIndicator number="2" title="SSI approval" active={authStep !== 'credentials'} done={authStep === 'approved'} /></div>
        {authStep === 'credentials' && <div className="space-y-4"><div className="flex flex-wrap items-center gap-2"><input ref={fileInputRef} type="file" accept=".json,application/json" className="hidden" onChange={event => void uploadJson(event.target.files?.[0])} /><ActionButton disabled={busy} onClick={() => fileInputRef.current?.click()}><Upload className="size-4" /> Upload JSON</ActionButton>{fileName && <span className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-emerald-300/10 bg-emerald-300/[0.04] px-3 text-xs text-emerald-200"><FileJson className="size-4" />{fileName}</span>}</div><div className="grid gap-3 sm:grid-cols-2"><Field label="Client ID"><input className="input" value={credentials.clientId} onChange={event => updateCredential('clientId', event.target.value)} placeholder="Client ID" autoComplete="off" /></Field><Field label="API Key"><input className="input" value={credentials.apiKey} onChange={event => updateCredential('apiKey', event.target.value)} placeholder="API Key" autoComplete="off" /></Field><Field label="API Secret"><input className="input" type="password" value={credentials.apiSecret} onChange={event => updateCredential('apiSecret', event.target.value)} placeholder="API Secret" autoComplete="new-password" /></Field><Field label="Private Key"><textarea className="input min-h-24 resize-y py-2" value={credentials.privateKey} onChange={event => updateCredential('privateKey', event.target.value)} placeholder="Private Key" autoComplete="off" /></Field></div><ActionButton disabled={busy} onClick={() => void requestOtp()}>{busy ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Request SSI approval</ActionButton></div>}
        {authStep === 'approval' && <div className="space-y-4"><div className="rounded-2xl border border-amber-300/15 bg-amber-300/[0.05] p-4"><div className="flex items-start gap-3">{approvalChecking ? <Loader2 className="mt-0.5 size-5 shrink-0 animate-spin text-amber-200" /> : <Clock3 className="mt-0.5 size-5 shrink-0 text-amber-200" />}<div className="min-w-0 flex-1"><p className="font-semibold text-amber-100">Waiting for SSI approval</p><p className="mt-1 text-sm leading-5 text-amber-100/70">Open SSI iBoard/app and approve the login request. This screen checks the approval automatically every 5 seconds.</p><p className="mt-3 break-all text-[11px] text-amber-100/50">Transaction ID: {transactionId}</p></div></div></div><div className="flex flex-wrap gap-2"><ActionButton disabled={approvalChecking || busy} onClick={() => void checkApproval(false)}>{approvalChecking ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Check approval</ActionButton><ActionButton disabled={busy} onClick={resetFlow}><RotateCcw className="size-4" /> Start over</ActionButton></div><Field label="OTP (if SSI asks for OTP)"><input className="input" inputMode="numeric" value={otp} onChange={event => { setOtp(event.target.value); setResult(null); }} placeholder="Enter OTP from SSI" autoComplete="one-time-code" /></Field></div>}
        {authStep === 'approved' && <div className="space-y-4"><div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-emerald-200" /><div><p className="font-semibold text-emerald-100">SSI approval confirmed</p><p className="mt-1 text-sm leading-5 text-emerald-100/70">The SSI authentication request was approved. Test Connection is now enabled.</p></div></div></div><div className="flex flex-wrap gap-2"><ActionButton disabled={busy} onClick={() => void testConnection()}>{busy ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Test Connection</ActionButton><ActionButton disabled={busy || !tested || !accountNo} onClick={save}>Save</ActionButton><ActionButton disabled={busy} onClick={resetFlow}><RotateCcw className="size-4" /> Start over</ActionButton></div>{accountNo && <p className="text-xs text-zinc-400">Selected SSI account: {accountNo}</p>}</div>}
...
function StepIndicator({ number, title, active, done }: { number: string; title: string; active: boolean; done: boolean }) { return <div className={`flex min-h-11 items-center gap-2 rounded-xl border px-3 ${done ? 'border-emerald-300/15 bg-emerald-300/[0.05]' : active ? 'border-violet-300/20 bg-violet-300/[0.06]' : 'border-violet-200/[0.08] bg-[#1b1123]'}`}>{done ? <CheckCircle2 className="size-4 text-emerald-200" /> : <span className="grid size-5 place-items-center rounded-full bg-violet-300/10 text-[10px] font-bold text-violet-100">{number}</span>}<span className="text-xs font-semibold">{title}</span></div>; }
function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.1em] text-[#81748a]">{label}</span>{children}</label>; }
function ActionButton({ disabled, onClick, children }: { disabled?: boolean; onClick: () => void; children: ReactNode }) { return <button type="button" disabled={disabled} onClick={onClick} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-violet-200/10 bg-[#1b1123] px-3.5 text-xs font-semibold text-[#ddd2e5] transition hover:bg-[#23152d] disabled:cursor-not-allowed disabled:opacity-45">{children}</button>; }
