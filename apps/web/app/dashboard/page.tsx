...
export default function DashboardPage() {
  ...
  if (authLoading || !initialized || !user) {
    return <main className="min-h-svh bg-[#090510] p-5 text-white"><Loading /></main>;
  }
    ...
    <main className="min-h-svh overflow-x-clip bg-[#090510] text-[#f4effa]">
        ...
        <div className="mx-auto max-w-6xl px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="grid size-9 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#d69cff] to-[#7d37c9]"><Activity className="size-4" /></div>
              <div className="min-w-0"><p className="text-[10px] uppercase tracking-[.14em] text-[#9d8fa8]">TCE account</p><p className="truncate text-sm font-semibold">{user.email}</p></div>
            </div>
          ...
          <nav className="mt-4 flex gap-6 border-b border-violet-200/[0.08] text-sm font-medium text-[#8e8299]">
            {(['Overview', 'Positions', 'Orders'] as Tab[]).map((item) => <button key={item} onClick={() => setTab(item)} className={`relative min-h-10 ${tab === item ? 'text-white' : ''}`}>{item}{tab === item && <span className="absolute inset-x-0 bottom-[-1px] h-0.5 bg-[#c06cff]" />}</button>)}
          </nav>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-12 pt-5 sm:px-6 sm:pt-7">
        <section className="mb-5 flex items-end justify-between gap-3">
          <div><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#887b91]">Portfolio</p><h1 className="mt-1 text-[24px] font-semibold tracking-[-.035em] sm:text-3xl">Investigate value / total</h1></div>
          <button onClick={() => void load()} disabled={loading} aria-label="Refresh dashboard" className="grid size-11 place-items-center rounded-xl border border-violet-200/10 bg-[#17101d] text-[#bcaec6] disabled:opacity-50"><RefreshCw className={`size-4 ${loading ? 'animate-spin' : ''}`} /></button>
        </section>

        <section className="mb-5 rounded-[22px] border border-violet-200/[0.09] bg-[#150d1d] p-5 shadow-[0_18px_50px_rgba(0,0,0,.2)]">
          <p className="text-xs text-[#8e8197]">Total portfolio value</p>
          <p className="mt-2 text-3xl font-semibold tracking-tight">{money(account.totalValue ?? account.portfolioValue ?? account.equity)}</p>
          <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <Metric label="Invested" value={money(account.investedCapital)} />
            <Metric label="P&L" value={money(account.unrealizedPnl ?? account.pnl)} />
            <Metric label="Positions" value={String(positions.length)} />
            <Metric label="Pools" value={String(pools.length)} />
          </div>
        </section>
