...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (requested === 'overview' || requested === 'positions' || requested === 'orders' || requested === 'settings')
      setTab(requested);
  }, []);
  useEffect(() => {
    if (user) void load();
    ...
    <main className="app-shell">
      ...
      <div className="app-container app-content">
        {tab !== 'settings' && (
          <section className="page-heading">
            ...
            <Button variant="outline" size="icon" className="touch-target shrink-0" onClick={() => void load()} disabled={loading} aria-label="Refresh dashboard">
          ...
          <section className="mb-5">
            ...
            <div className="account-strip">
              {visibleAccounts.slice(0, 6).map((item: any, index: number) => (
                <div key={item.id ?? item.accountNo ?? item.externalAccountNo ?? index} className="account-chip">
                  <span className="account-chip-icon">{String(item.provider ?? item.broker ?? 'ACC').slice(0, 1).toUpperCase()}</span>
                  <span className="min-w-0">
                    <strong>{String(item.provider ?? item.broker ?? 'Account')}</strong>
                    <small>{String(item.accountNo ?? item.externalAccountNo ?? item.environment ?? 'Connected')}</small>
                  </span>
        ...
        {tab === 'overview' && (
          <div className="dashboard-sections">
            <Panel title="Current Positions" caption={`${positions.length} assets`} icon={WalletCards} action={() => { setTab('positions'); router.replace('/dashboard?tab=positions', { scroll: false }); }}><AssetList rows={positions} /></Panel>
            <Panel title="Shared Pools" caption={`${pools.length} watching`} icon={Layers3}><AssetList rows={pools} kind="pool" /></Panel>
            <Panel title="Next Positions" caption={next.length ? `${next.length} candidates` : 'Candidates'} icon={TrendingUp}><AssetList rows={next} kind="candidate" /></Panel>
            <Panel title="Recent Orders" caption={`${orders.length} orders`} icon={ShoppingCart} action={() => { setTab('orders'); router.replace('/dashboard?tab=orders', { scroll: false }); }}><AssetList rows={orders} /></Panel>
          </div>
        )}
      ...
      <nav aria-label="Dashboard navigation" className="mobile-bottom-nav">
        {navigation.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button key={id} type="button" onClick={() => selectNavigation(id)} aria-current={active ? 'page' : undefined} className={`bottom-nav-item ${active ? 'is-active' : ''}`}>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div><p className="metric-label">{label}</p><p className="metric-secondary">{value}</p></div>;
}
function SectionHeader({ title, caption }: { title: string; caption: string }) {
  return <div className="section-header"><h2>{title}</h2><span>{caption}</span></div>;
}
  ...
  return (
    <Card className="panel-card">
      <div className="panel-head">
        <div className="flex min-w-0 items-center gap-3"><div className="panel-icon"><Icon className="size-4" /></div><div className="min-w-0"><h2>{title}</h2><p>{caption}</p></div></div>
        {action && <button className="panel-action" onClick={action}>View all<ChevronRight className="size-4" /></button>}
      </div>
  ...
  return (
    <div className="asset-list">
      {rows.slice(0, 6).map((row, i) => {
        ...
        const secondary = isPool ? `#${rank ?? '—'} · ${score == null ? '—' : formatNumber(score)} · ${String(row.status ?? 'WATCHING')}` : isCandidate ? `#${rank ?? '—'} · ${String(row.status ?? 'queued').toUpperCase()}` : String(row.accountNo ?? row.status ?? row.type ?? 'OPEN');
        const tertiary = isPool ? entryLow != null && entryHigh != null ? `Entry ${formatNumber(entryLow)}–${formatNumber(entryHigh)}${targetPrice != null ? ` · Target ${formatNumber(targetPrice)}` : ''}` : targetPrice != null ? `Target ${formatNumber(targetPrice)}` : null : isCandidate && quantity != null ? `Qty ${formatNumber(quantity)}` : null;
        return (
          <div key={row.id ?? `${symbol}-${i}`} className="asset-row">
            <div className="asset-mark">{symbol.slice(0, 2).toUpperCase()}</div>
            <div className="asset-main"><p>{symbol}</p><small>{secondary}</small>{tertiary && <small>{tertiary}</small>}</div>
            <div className="asset-value"><strong>{isPool ? `Price ${primaryValue}` : isCandidate ? primaryValue : formatQuantity(quantity)}</strong><small>{isPool || isCandidate ? '' : money(row.marketValue ?? row.market_value ?? row.price)}</small></div>
...
function MobileDataView({ title, caption, rows, columns }: { title: string; caption: string; rows: any[]; columns: string[] }) {
  return (
    <section>
      ...
      <Card className="data-card"><div className="mobile-records">{rows.map((row, i) => <div className="mobile-record" key={row.id ?? i}>{columns.map((column, index) => <div key={column} className={index === 0 ? 'record-primary' : 'record-field'}><span>{column.replace(/[A-Z]/g, m => ` ${m}`).toUpperCase()}</span><strong>{String(row?.[column] ?? '—')}</strong></div>)}</div>)}</div>{!rows.length && <Empty />}</Card>
    </section>
  );
}
function Empty({ kind = 'default' }: { kind?: 'default' | 'pool' | 'candidate' }) {
  return <div className="empty-state"><Layers3 className="size-5" /><span>{kind === 'candidate' ? 'No promoted candidates' : kind === 'pool' ? 'No watching pool entries' : 'No data yet'}</span></div>;
}
function Loading() {
  return <div className="loading-state"><div className="brand-orb"><Activity className="size-4" /></div><div><strong>Opening TCE</strong><span>Checking secure session…</span></div></div>;
}
...
function formatNumber(value: unknown) {
  if (value == null || value === '') return '—';
  const n = Number(value);
  return Number.isFinite(n) ? n.toLocaleString('vi-VN', { maximumFractionDigits: 2 }) : String(value);
}
function formatQuantity(value: unknown) {
  if (value == null || value === '') return '—';
  const n = Number(value);
  return Number.isFinite(n) ? n.toLocaleString('vi-VN', { maximumFractionDigits: 4 }) : String(value);
}
