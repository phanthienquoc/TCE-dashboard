...
export default function StockEventsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return Array.from(
      new Map(
        rows.map(event => [`${event.ticker}|${event.exDividendDate}|${event.eventContent}`, event])
      ).values()
    );
  }, [events, query]);
      ...
      <div className="mx-auto w-full max-w-6xl space-y-4 pb-4">
        <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Upcoming dividend events from the live stock-dividend database.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          <div className="rounded-xl border p-4">
            <div className="text-xs text-muted">Upcoming events</div>
            <div className="mt-1 text-2xl font-semibold">{filtered.length}</div>
          </div>
          <div className="rounded-xl border p-4">
            <div className="text-xs text-muted">Stocks</div>
            <div className="mt-1 text-2xl font-semibold">{uniqueTickers}</div>
          </div>
          <div className="hidden rounded-xl border p-4 md:block">
            ...
            <div className="mt-1 text-sm font-medium">vietstock.events</div>
          </div>
        </div>
        ...
        <section className="overflow-hidden rounded-xl border">
          ...
          {!loading && !error && filtered.length === 0 ? (
            <div className="p-8 text-sm text-muted">No upcoming events.</div>
          ) : null}
          {!loading && !error && filtered.length > 0 ? (
            <div className="divide-y">
              {filtered.map(event => {
                ...
                return (
                  <article
                    key={event.id}
                    className="grid gap-3 p-4 md:grid-cols-[90px_110px_1fr_130px] md:items-center"
                  >
                    <div>
                      <div className="font-semibold">{event.ticker}</div>
                      <div className="text-xs text-muted">{event.exDividendDate || '—'}</div>
                    </div>
                    <div>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-muted">{event.dividendRate || '—'}</div>
                    </div>
                    <div className="md:text-right">
                      <div className="font-semibold">
                        {event.dividendValue ? event.dividendValue.toLocaleString('vi-VN') : '—'}
                      </div>
                      <div className="text-xs text-muted">dividend / event</div>
                    </div>
                  </article>
                );
              })}
