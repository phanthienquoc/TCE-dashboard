...
export default function Dashboard(){
  ...
  return <main className="dashboard-shell min-h-svh overflow-x-clip bg-[#070b12] text-slate-100">
    <header className="dashboard-header sticky top-0 z-40 border-b border-white/[0.08] bg-[#070b12]/92 backdrop-blur-xl">
      <div className="dashboard-container flex min-h-14 items-center justify-between gap-3 px-4 py-2 sm:min-h-16 sm:px-6">
        <div className="min-w-0">
          <div className="flex items-center gap-2"><div className="grid size-8 shrink-0 place-items-center rounded-lg bg-white text-slate-950"><Activity className="size-4"/></div><h1 className="truncate text-base font-semibold tracking-tight">TCE</h1></div>
          <p className="hidden truncate pl-10 text-xs text-slate-500 sm:block">Treasury Cash Extraction</p>
        </div>
    ...
    <div className="dashboard-container px-4 pb-[calc(92px+env(safe-area-inset-bottom))] pt-4 sm:px-6 sm:pb-10 sm:pt-7">
      <section className="mb-5 sm:mb-6">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[.14em] text-slate-500">Portfolio</p>
            <h2 className="mt-1 text-[25px] font-semibold leading-tight tracking-[-.03em] sm:text-3xl">Investigate value / total</h2>
            <p className="mt-1 truncate text-xs text-slate-500">{user.email}</p>
          </div>
          <button onClick={load} disabled={loading} aria-label="Refresh dashboard" className="touch-target inline-flex shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-300 shadow-sm transition hover:bg-white/[0.08] disabled:opacity-50"><RefreshCw className={`size-4 ${loading?'animate-spin':''}`}/></button>
        </div>
      </section>

      <section className="mb-6 grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="hero-card rounded-2xl border border-white/[0.1] bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,.09),transparent_42%),linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.025))] p-4 shadow-2xl shadow-black/20 sm:p-5">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-400">Portfolio value</p>
              <p className="mt-2 truncate text-3xl font-semibold tracking-[-.04em] sm:text-4xl">{formatMoney(total??invested)}</p>
              <p className="mt-2 text-xs text-slate-500">{invested!=null?`Invested ${formatMoney(invested)}`:'Value not available yet'}</p>
            </div>
            <div className="grid size-11 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.05]"><CircleDollarSign className="size-5 text-slate-300"/></div>
          </div>
          <div className="mt-5 flex items-center gap-2 text-sm"><span className="rounded-full bg-white/[0.06] px-2.5 py-1 text-xs text-slate-400">{positions.length} active positions</span><span className={Number(pnl)>=0?'text-emerald-300':'text-rose-300'}>{pnl!=null?formatMoney(pnl):'P&L —'}</span></div>
        </section>
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2">
          <Metric label="Pools" value={pools.length} icon={Layers3}/><Metric label="Positions" value={positions.length} icon={WalletCards}/><Metric label="Next" value={next.length} icon={TrendingUp}/><Metric label="Orders" value={orders.length} icon={ShoppingCart}/>
        </section>
      </section>

      <div className="desktop-tabs mb-5 overflow-x-auto rounded-xl border border-white/[0.08] bg-white/[0.025] p-1 backdrop-blur-xl">
        <nav className="flex min-w-max gap-1" aria-label="Dashboard sections">{tabs.map(([label,Icon])=><button key={label} onClick={()=>selectTab(label)} className={`touch-target inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-medium transition active:scale-[.98] sm:text-sm ${tab===label?'bg-white text-slate-950 shadow-sm':'text-slate-500 hover:bg-white/[0.06] hover:text-slate-200'}`}><Icon className="size-4"/><span>{label}</span></button>)}</nav>
      </div>
    ...
    <nav className="mobile-bottom-nav fixed inset-x-0 bottom-0 z-50 border-t border-white/[0.08] bg-[#070b12]/95 px-2 pt-2 backdrop-blur-xl" aria-label="Mobile navigation">
      <div className="mx-auto grid max-w-md grid-cols-4 gap-1 pb-[calc(8px+env(safe-area-inset-bottom))]">{nav.map(([label,Icon])=><button key={label} onClick={()=>selectTab(label)} aria-current={tab===label?'page':undefined} className={`touch-target flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-xl text-[10px] font-medium transition active:scale-[.97] ${tab===label?'bg-white text-slate-950':'text-slate-500'}`}><Icon className="size-4"/><span>{label==='Recent Orders'?'Orders':label}</span></button>)}</div>
    </nav>
  </main>
}
