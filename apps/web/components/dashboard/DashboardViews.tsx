    ...
    <div className="tce-mobile-view">
      ...
      <MobileSection title="Capital Pools" action="Engine" href="/engines">
        <div className="tce-pool-balance-grid">
          {poolStates.map(pool => (
            <article
              className={`tce-pool-balance ${pool.active ? 'is-active' : pool.available > 0 ? 'is-ready' : ''}`}
              key={pool.pool}
            >
              ...
              <div className="tce-pool-balance-amount">
                ...
                <span className="tce-pool-balance-state">
                  {pool.pending ? 'T+2' : pool.active ? 'active' : 'ready'}
                </span>
              </div>
              ...
              <div className="tce-pool-balance-meta">
                <span>{pool.active ? `used ${money(pool.used)}` : 'unused'}</span>
...
export function PositionsView({ data, actions }: ViewProps) {
  ...
  const dividendMonthGroups = useMemo(() => {
    ...
    for (const event of events) {
      ...
      month.set(ticker, [...(month.get(ticker) ?? []), event]);
    ...
    return [...grouped.entries()]
      ...
      .sort((a, b) => dividendMonthIndex(a.monthKey) - dividendMonthIndex(b.monthKey));
  }, [events]);
            ...
            dividendMonthGroups.map(group => (
              <section className="tce-dividend-month-group" key={group.monthKey}>
                <button
                  ...
                  aria-expanded={!collapsedDividendMonths.has(group.monthKey)}
                  onClick={() =>
                ...
                >
                  ...
                  <ChevronDown
                    className={`tce-dividend-month-chevron${collapsedDividendMonths.has(group.monthKey) ? '' : ' is-open'}`}
                  />
                </button>
                {!collapsedDividendMonths.has(group.monthKey) && (
                    ...
                    {group.cards.map(item => {
                      ...
                      return (
                        <DividendCard
                          ...
                          expanded={expandedDividend === key}
                          onToggle={() =>
                            setExpandedDividend(current => (current === key ? null : key))
                          }
                          onOrder={() =>
                            actions.openTrade({
                              ...pool,
...
function MiniChart({ positive = true }: { positive?: boolean }) {
  return (
    <svg className="tce-mini-chart" viewBox="0 0 120 42" aria-hidden="true">
      <path
        d={
          positive
            ? 'M2 34 C 20 31, 22 27, 35 29 S 54 23, 65 25 S 83 12, 95 16 S 108 8, 118 5'
            : 'M2 8 C 18 11, 28 7, 39 13 S 58 12, 70 20 S 91 19, 102 29 S 111 31, 118 36'
        }
        fill="none"
        stroke="currentColor"
        strokeWidth="2.5"
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="tce-muted">
            {formatDate(event?.gdkhqTimestamp ?? event?.gdkhq_timestamp ?? event?.exDate)}
          </span>
        ...
        <div className="tce-pool-grid mt-2">
          <div>
            <span>Dividend</span>
            <b>{dividendValue ? `${money(dividendValue)} ₫` : '—'}</b>
          </div>
          <div>
            <span>Price</span>
          ...
          <div>
            <span>TP</span>
            <b>{formatNumber(targetPrice)}</b>
          </div>
        </div>
      </button>
      {expanded && (
        <div className="tce-card-actions mt-3">
          <span className="text-xs">Entry {formatEntry(entryLow, entryHigh)}</span>
...
function RotationRow({ row }: { row: any }) {
  ...
  const invested = Number(row.costBasis ?? row.cost_basis ?? (entry > 0 ? entry * quantity : 0));
  return (
    <article className="tce-rotation-row">
      ...
      <div className="tce-rotation-footer">
        ...
        <span>{formatT2Label(sellable)}</span>
      </div>
    </article>
  );
}
...
function formatT2Label(value: any) {
  const raw = String(value ?? '').trim();
  if (!raw) return 'T+2';
  if (raw.toUpperCase() === 'SELLABLE') return 'SELLABLE';
  return raw.length > 22 ? `${raw.slice(0, 22)}…` : raw;
}
function symbolOf(row: any) {
  return String(row?.symbol ?? row?.code ?? row?.ticker ?? '—').toUpperCase();
}
function dividendEventDate(event: StockEvent) {
  const value = event.gdkhqTimestamp ?? event.gdkhq_timestamp ?? event.exDate;
  if (!value) return null;
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? null : date;
}
function dividendMonthIndex(monthKey: string) {
  const [year, month] = monthKey.split('-').map(Number);
  return year * 12 + (month - 1);
}
function dividendMonthLabel(monthKey: string) {
  const [year, month] = monthKey.split('-').map(Number);
