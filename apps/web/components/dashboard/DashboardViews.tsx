...
export function PositionsView({ data, actions }: { data: DashboardData; actions: DashboardActions }) {
  ...
  return (
    <div className="dashboard-view dashboard-view-positions">
      ...
      <Panel title="Next Positions" caption={data.next.length ? `${data.next.length} candidates` : 'Candidates'} icon={TrendingUp}>
        <AssetList rows={data.next} kind="candidate" onTrade={actions.openNextPositionOrder} onReturn={actions.returnNextPositionToPool} returnBusy={actions.returnBusy} />
      </Panel>

      <Panel
        title="Stock Dividend Pools"
        caption={stockEventsLoading ? 'Loading…' : dividendPools.length ? `${dividendPools.length} stocks` : 'Upcoming dividend events'}
        icon={Layers3}
      >
        {stockEventsError ? <div className="empty-state">{stockEventsError}</div> : <DividendPoolList rows={dividendPools} />}
      </Panel>

      <Panel title="Shared Pools" caption={`${data.pools.length} watching`} icon={Layers3}>
        <AssetList rows={data.pools} kind="pool" onTrade={actions.openTrade} onPromote={actions.promotePool} promoteBusy={actions.promoteBusy} />
      </Panel>
    </div>
  );
}
...
export function SettingsView() {
  return (
    <div className="dashboard-view dashboard-view-settings">
      ...
      <div className="settings-grid-items"><PlatformConfigTab /></div>
    </div>
  );
}
    ...
    <Card className="panel-card">
      <div className="panel-head">
        <div className="flex min-w-0 items-center gap-3">
          <div className="panel-icon"><Icon className="size-4" /></div>
          <div className="min-w-0"><h2>{title}</h2><p>{caption}</p></div>
        </div>
...
function DividendPoolList({ rows }: { rows: Array<{ id: string; symbol: string; eventCount: number; nextExDate?: string }> }) {
  ...
  return <ListView items={rows.slice(0, 5).map(row => ({
    id: row.id,
    title: row.symbol,
...
function AssetList({ rows, kind = 'default', onTrade, onPromote, promoteBusy, onReturn, returnBusy }: { rows: any[]; kind?: 'default' | 'pool' | 'candidate' | 'position'; onTrade?: (row: any) => void; onPromote?: (row: any) => void; promoteBusy?: string | null; onReturn?: (row: any) => void; returnBusy?: string | null }) {
  ...
  const items = rows.slice(0, 4).map((row, i) => {
    ...
    const primaryValue = isCandidate && targetPrice != null ? `TP ${formatNumber(targetPrice)}` : isPool ? `Mkt ${formatNumber(currentPrice)}` : money(row.marketValue ?? row.market_value ?? row.price);
  ...
  const actionable = items.map((item, index) => ({ ...item, trailing: (
    <div className="asset-actions flex items-center gap-2">
      {item.trailing}
      {kind === 'candidate' && onTrade ? <button type="button" className="primary-action" onClick={() => onTrade(rows[index])}>BUY</button> : null}
      {kind === 'candidate' && onReturn && rows[index]?.id ? <button type="button" className="panel-action" onClick={() => onReturn(rows[index])} disabled={returnBusy === String(rows[index].id)}>{returnBusy === String(rows[index].id) ? '…' : 'To Pool'}</button> : null}
      {kind === 'pool' && onTrade ? <button type="button" className="panel-action" onClick={() => onTrade(rows[index])}>BUY</button> : null}
      {kind === 'position' && onTrade ? <button type="button" className="panel-action" onClick={() => onTrade(rows[index])}>SELL</button> : null}
      {kind === 'pool' && onPromote && rows[index]?.id ? <button type="button" className="panel-action" onClick={() => onPromote(rows[index])} disabled={promoteBusy === String(rows[index].id)}>{promoteBusy === String(rows[index].id) ? '…' : 'Promote'}</button> : null}
    </div>
  ) }));
