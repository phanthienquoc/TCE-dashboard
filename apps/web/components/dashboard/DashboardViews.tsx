...
import { Bell, CheckCircle2, ChevronRight, CircleDot, Layers3, Search, Settings2, ShieldCheck, TrendingUp, WalletCards, Wifi } from 'lucide-react';
...
export function PoolsView({ data, actions }: ViewProps) {
  return <div className="tce-mobile-view"><MobileHeader title="Pools" subtitle="TCE opportunities" icon={<Layers3 className="size-5" />} /><div className="tce-filter-row"><button className="tce-filter active">All</button><button className="tce-filter">Watchlist</button><button className="tce-filter">High Conviction</button></div><div className="tce-list-stack">{data.pools.length ? data.pools.map((row, index) => <PoolCard key={row.id ?? row.symbol ?? index} row={row} onBuy={() => actions.openTrade(row)} onPromote={() => actions.promotePool(row)} busy={actions.promoteBusy === String(row.id)} />) : <EmptyState text="No shared pool items" />}</div></div>;
}
...
export function PositionsView({ data, actions }: ViewProps) {
  ...
  const dividendPools = useMemo(() => {
    ...
    for (const event of events) { const ticker = String(event.ticker ?? '').trim().toUpperCase(); if (!ticker) continue; grouped.set(ticker, [...(grouped.get(ticker) ?? []), event]); }
    return [...grouped.entries()].map(([symbol, tickerEvents]) => ({ symbol, events: tickerEvents })).sort((a, b) => a.symbol.localeCompare(b.symbol));
  }, [events]);
  return <div className="tce-mobile-view"><MobileHeader title="Positions" subtitle="Live exposure" icon={<WalletCards className="size-5" />} live /><div className="tce-segmented">{(['current', 'dividend', 'history'] as const).map(item => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>{tab === 'current' && <div className="tce-list-stack">{data.positions.length ? data.positions.map((row, index) => <PositionCard key={row.id ?? row.symbol ?? index} row={row} onSell={() => actions.openPositionSell(row)} />) : <EmptyState text="No current positions" />}</div>}{tab === 'dividend' && <div className="tce-list-stack">{loading ? <EmptyState text="Loading dividend events…" /> : error ? <EmptyState text={error} /> : dividendPools.length ? dividendPools.map(item => <DividendCard key={item.symbol} item={item} pool={data.pools.find(p => String(p.symbol ?? p.code ?? '').toUpperCase() === item.symbol)} />) : <EmptyState text="No upcoming dividend pools" />}</div>}{tab === 'history' && <EmptyState text="Position history is ready for the next history feed." />}</div>;
}
...
function PoolCard({ row, onBuy, onPromote, busy }: { row: any; onBuy: () => void; onPromote: () => void; busy: boolean }) { const score = Number(row.score ?? 0); const price = row.currentPrice ?? row.current_price ?? row.marketPrice ?? row.market_price; return <article className="tce-pool-card"><div className="tce-card-top"><div><strong>{symbolOf(row)}</strong><span>{String(row.name ?? 'TCE Pool')}</span></div><div className="tce-score"><strong>{Number.isFinite(score) && score ? score : '—'}</strong><span>TCE Score</span></div></div><div className="tce-price-line"><strong>{formatNumber(price)}</strong><span className="tce-positive">{String(row.status ?? 'WATCHING')}</span></div><MiniChart positive /><div className="tce-pool-grid"><div><span>Entry</span><b>{formatEntry(row.entryLow ?? row.entry_low, row.entryHigh ?? row.entry_high)}</b></div><div><span>TP</span><b>{formatNumber(row.targetPrice ?? row.target_price)}</b></div><div><span>R:R</span><b>{String(row.rr ?? row.riskReward ?? row.risk_reward ?? '—')}</b></div></div><div className="tce-card-actions"><button onClick={onBuy}>BUY</button><button onClick={onPromote} disabled={busy}>{busy ? '…' : 'Promote'}</button></div></article>; }
...
function DividendCard({ item, pool }: { item: { symbol: string; events: StockEvent[] }; pool?: any }) { const event = item.events[0]; return <article className="tce-dividend-card"><div><strong>{item.symbol}</strong><span>Dividend Pool</span></div><div className="tce-dividend-info"><span>Next ex-date</span><b>{formatDate(event?.exDividendDate)}</b><span>Rate</span><b>{String(event?.dividendRate ?? '—')}</b></div><div className="tce-pool-grid"><div><span>Price</span><b>{formatNumber(pool?.currentPrice ?? pool?.current_price)}</b></div><div><span>Entry</span><b>{formatEntry(pool?.entryLow ?? pool?.entry_low, pool?.entryHigh ?? pool?.entry_high)}</b></div><div><span>TP</span><b>{formatNumber(pool?.targetPrice ?? pool?.target_price)}</b></div></div></article>; }
function SettingRow({ icon, label, value, ok }: { icon: React.ReactNode; label: string; value: string; ok?: boolean }) { return <div className="tce-setting-row"><span className="tce-setting-icon">{icon}</span><strong>{label}</strong><span className={ok ? 'tce-positive' : ''}>{value}</span></div>; }
function EmptyState({ text }: { text: string }) { return <div className="tce-empty">{text}</div>; }
function MiniChart({ positive }: { positive: boolean }) { return <svg className={positive ? 'tce-mini-chart positive' : 'tce-mini-chart'} viewBox="0 0 120 42" aria-hidden="true"><polyline points="2,32 18,28 31,30 44,19 57,24 71,15 84,18 98,8 118,11" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg>; }
function symbolOf(row: any) { return String(row.symbol ?? row.code ?? row.name ?? '—').trim().toUpperCase(); }
...
function signedMoney(value: any) { const n = number(value); return `${n >= 0 ? '+' : '-'}${money(Math.abs(n))}`; }
...
function formatDate(value?: string) { if (!value) return '—'; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString('vi-VN'); }
function timeNow() { return new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }); }
