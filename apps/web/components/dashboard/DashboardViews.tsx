...
import { ArrowLeftRight, ChevronRight, Layers3, ShoppingCart, TrendingUp, WalletCards, X } from 'lucide-react';
...
export function PositionsView({ data }: { data: DashboardData }) {
  return <CompactDataView title="Positions" caption="Current asset exposure" rows={data.positions} columns={['symbol', 'quantity', 'avgBuyCost', 'marketPrice', 'unrealizedPnl']} />;
}

export function OrdersView({ data }: { data: DashboardData }) {
  return <CompactDataView title="Orders" caption="Execution history" rows={data.orders} columns={['side', 'symbol', 'quantity', 'price', 'status', 'fee', 'tax']} />;
}

export function SettingsView() {
  return <div className="dashboard-view dashboard-view-settings"><PlatformConfigTab /></div>;
}
  ...
  return (
    <Card className="panel-card">
      <div className="panel-head">
        ...
        {action && <button type="button" className="panel-action" onClick={action}>View all <ChevronRight className="size-4" /></button>}
      </div>
...
function AssetList({ rows, kind = 'default', onTrade, onPromote, promoteBusy }: { rows: any[]; kind?: 'default' | 'pool' | 'candidate'; onTrade?: (row: any) => void; onPromote?: (row: any) => void; promoteBusy?: string | null }) {
  ...
  return <div className="asset-list">{rows.slice(0, 4).map((row, i) => {
    ...
    return <div key={row.id ?? row.symbol ?? row.code ?? i} className="asset-row">
      <div className="asset-main"><div className="asset-symbol">{symbol}</div><div className="asset-secondary">{secondary}</div></div>
      <div className="asset-value">{primaryValue}</div>
      {isCandidate && onTrade && <div className="asset-actions"><button type="button" className="primary-action" onClick={() => onTrade(row)}>Create order</button></div>}
      {isPool && <div className="asset-actions">
        {onTrade && <button type="button" className="panel-action" onClick={() => onTrade(row)}>Trade</button>}
        {onPromote && row.id && <button type="button" className="panel-action" onClick={() => onPromote(row)} disabled={promoteBusy === String(row.id)}>{promoteBusy === String(row.id) ? '…' : 'Promote'}</button>}
      </div>}
    </div>;
  })}</div>;
}

function CompactDataView({ title, caption, rows, columns }: { title: string; caption: string; rows: any[]; columns: string[] }) {
  return <section className="mobile-data-view"><SectionHeader title={title} caption={`${rows.length} ${rows.length === 1 ? 'item' : 'items'}`} /><Card className="data-card"><div className="mobile-records">{rows.map((row, index) => <div className="mobile-record" key={row.id ?? row.symbol ?? index}><div className="mobile-record-grid">{columns.map((column, columnIndex) => <div key={column} className={columnIndex === 0 ? 'record-primary' : 'record-field'}><span>{column}</span><strong>{String(getValue(row, column) ?? '—')}</strong></div>)}</div></div>)}</div>{rows.length > 5 && <div className="list-footer"><span>Scroll to view all {rows.length}</span></div>}</Card></section>;
}
