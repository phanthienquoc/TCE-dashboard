...
export function OverviewView({ data, actions }: { data: DashboardData; actions: DashboardActions }) {
  ...
  return (
    <div className="dashboard-view dashboard-view-overview">
      <Card className="hero-card"><CardContent className="p-4"><div className="hero-card-top"><div><p className="metric-label">Total portfolio value</p><p className="metric-value">{money(portfolioValue)}</p></div><div className="hero-status">LIVE</div></div><div className="metric-grid"><Metric label="Invested" value={money(invested)} /><Metric label="Positions" value={String(positions.length)} /><Metric label="Accounts" value={String(visibleAccounts.length || '—')} /></div></CardContent></Card>
      {visibleAccounts.length > 0 && <section className="dashboard-accounts"><SectionHeader title="Connected accounts" caption={`${visibleAccounts.length} connected`} /><div className="account-strip shared-account-strip">{visibleAccounts.map((item: any, index: number) => <AccountCard key={item.id ?? item.accountNo ?? item.externalAccountNo ?? index} provider={String(item.provider ?? item.broker ?? 'Account')} identifier={String(item.accountNo ?? item.externalAccountNo ?? item.environment ?? 'Connected')} status={String(item.status ?? 'Connected')} />)}</div></section>}
...
function rowsToListItems(rows: any[], type: 'position' | 'order') {
  return rows.map((row, index) => {
    ...
    const values = type === 'position'
      ? [`Qty ${formatNumber(row.quantity ?? 0)}`, `Avg ${formatNumber(row.avgBuyCost)}`, `Mkt ${formatNumber(row.marketPrice)}`, `P&L ${formatNumber(row.unrealizedPnl)}`]
      : [String(row.side ?? '—'), `Qty ${formatNumber(row.quantity ?? 0)}`, `Price ${formatNumber(row.price)}`, String(row.status ?? '—'), `Fee ${formatNumber(row.fee)}`, `Tax ${formatNumber(row.tax)}`];
    return { id: row.id ?? row.symbol ?? row.code ?? index, title: symbol, description: values.join(' · '), meta: type === 'position' ? 'POSITION' : 'ORDER' };
  });
}

export function PositionsView({ data }: { data: DashboardData }) {
  return <section className="dashboard-data-section"><SectionHeader title="Positions" caption={`${data.positions.length} ${data.positions.length === 1 ? 'item' : 'items'}`} /><ListView items={rowsToListItems(data.positions, 'position')} /></section>;
}

export function OrdersView({ data }: { data: DashboardData }) {
  return <section className="dashboard-data-section"><SectionHeader title="Orders" caption={`${data.orders.length} ${data.orders.length === 1 ? 'item' : 'items'}`} /><ListView items={rowsToListItems(data.orders, 'order')} /></section>;
}
