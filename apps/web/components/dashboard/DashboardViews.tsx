...
function buildPoolStates(capital: number, availableCash: number, pendingCash: number, positions: any[], pools: any[]) {
  ...
  for (const row of positions) { const pool = String(row.pool ?? row.pool_id ?? '').toUpperCase(); if (poolRows.has(pool)) poolRows.get(pool)!.push(row); }
  ...
  return poolIds.map((pool, index) => {
    ...
    return { pool, allocated, available: poolAvailable, active: rows.length, slots: 1, pending: poolPending, usedRatio: allocated > 0 ? Math.min(1, (usedByPool.get(pool) ?? 0) / allocated) : 0 };
  });
}
