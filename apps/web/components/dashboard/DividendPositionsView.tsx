...
import { PriceCombobox } from '../ui/PriceCombobox';
...
const DEFAULT_PRICE_FILTER: number | null = null;
...
export function DividendPositionsView({ data, actions }: ViewProps) {
  ...
  const [priceFilter, setPriceFilter] = useState<number | null>(DEFAULT_PRICE_FILTER);
  ...
  return <div className="tce-mobile-view">
    ...
    {tab === 'dividend' && <div className="tce-list-stack tce-dividend-month-groups">
      <div className="tce-dividend-filter-bar" role="group" aria-label="Dividend filters">
        ...
        <label className="tce-dividend-filter-field" htmlFor="positions-current-price"><Tag className="size-4" aria-hidden="true" /><span className="w-full"><small>Current price (VND)</small><PriceCombobox value={priceFilter} onChange={value => { setPriceFilter(value); setExpanded(null); }} /></span></label>
      </div>
      ...
      })}</div></section>) : <EmptyState text={`No dividend events scheduled for ${dividendMonthLabel(selectedMonth)}${priceFilter == null ? '' : ` under ${priceFilter.toLocaleString('vi-VN')} VND`}`} />}
    </div>}
...
function buildFutureMonthGroups(events: StockEvent[], maxPrice: number | null, marketPrices: Record<string, { price?: number | null }>): MonthGroup[] {
  ...
  for (const event of events) { const date = dividendEventDate(event); const symbol = String(event.ticker ?? '').trim().toUpperCase(); if (!date || !symbol || date.getTime() < today) continue; const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; if (!monthKeys.has(monthKey)) continue; const live = marketPrices[symbol]?.price; const price = Number(live) > 0 ? Number(live) : Number(event.currentPrice ?? event.price ?? 0); if (maxPrice != null && (price <= 0 || price > maxPrice)) continue; const bucket = grouped.get(monthKey) ?? new Map<string, StockEvent[]>(); bucket.set(symbol, [...(bucket.get(symbol) ?? []), event]); grouped.set(monthKey, bucket); }
