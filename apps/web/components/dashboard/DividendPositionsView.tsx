...
export function DividendPositionsView({ data, actions }: ViewProps) {
  ...
  return (
    <div className="tce-mobile-view">
      ...
      {tab === 'current' && <div className="tce-list-stack">
        {data.positions.length ? data.positions.map((row, index) => <article className="tce-dividend-card" key={row.id ?? row.symbol ?? index}>
