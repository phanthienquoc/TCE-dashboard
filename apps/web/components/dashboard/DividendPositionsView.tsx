        ...
        <div className="tce-list-stack">
          {data.positions.length ? (
            data.positions.map((row, i) => (
              <article className="tce-dividend-card" key={row.id ?? row.symbol ?? i}>
                <div className="flex items-center gap-2">
                  <strong>{String(row.symbol ?? '—')}</strong>
                  <span className="tce-muted">Current position</span>
                </div>
                <div className="tce-pool-grid mt-2">
                  ...
                  <div>
                    <span>P&L</span>
                    <b>{formatNumber(row.pnl ?? row.unrealizedPnl ?? row.unrealized_pnl)}</b>
                  </div>
                </div>
                <button
                  className="tce-secondary-action mt-3"
                  onClick={() => actions.openPositionSell(row)}
                >
                  SELL
                </button>
              </article>
        ...
        <div className="tce-list-stack tce-dividend-month-groups">
          <div className="tce-dividend-filter-bar" role="group" aria-label="Dividend filters">
            <label className="tce-dividend-filter-field" htmlFor="positions-dividend-month">
              ...
              <span>
                <small>Ex-date month</small>
                <select
                  id="positions-dividend-month"
                  value={selectedMonth}
                  onChange={e => {
                    setSelectedMonth(e.target.value);
                    setExpanded(null);
                  }}
                  aria-label="Ex-date month"
                >
                  {monthOptions.map(m => (
            ...
            <label className="tce-dividend-filter-field" htmlFor="positions-current-price">
              ...
              <span>
                <small>Current price (VND)</small>
                <select
                  id="positions-current-price"
                  value={priceFilter}
                  onChange={e => {
                    setPriceFilter(Number(e.target.value));
                    setExpanded(null);
                  }}
                  aria-label="Current price filter"
                >
                  {PRICE_OPTIONS.map(v => (
          ...
          {loading ? (
            ...
            <EmptyState
              text={`No dividend events scheduled for ${dividendMonthLabel(selectedMonth)} under ${priceFilter.toLocaleString('vi-VN')} VND`}
            />
          )}
        </div>
      )}
