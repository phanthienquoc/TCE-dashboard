...
export function DividendOneYearCandleChart({ symbol }: Props) {
  ...
  const chart = useMemo(() => {
    ...
    return { valid, min, max, candles: valid.map((row, index) => ({ x: left + index * step, openY: y(row.open!), closeY: y(row.close!), highY: y(row.high!), lowY: y(row.low!), up: row.close! >= row.open! } as Candle)) };
  }, [rows]);

  if (loading && !rows.length) return <div className="mt-2 rounded-lg border border-white/[0.06] bg-black/10 px-2 py-3 text-center text-[9px] text-slate-500">Loading chart…</div>;
  if (error) return <div className="mt-2 rounded-lg border border-white/[0.06] bg-black/10 px-2 py-3 text-center text-[9px] text-slate-500">{error}</div>;
  if (!chart) return <div className="mt-2 rounded-lg border border-white/[0.06] bg-black/10 px-2 py-3 text-center text-[9px] text-slate-500">No 1Y history</div>;
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-lg border border-white/[0.06] bg-black/10 px-2 pt-2 pb-1">
      <div className="flex items-center justify-between px-1">
        ...
        {changePct != null && <span className={changePct >= 0 ? 'text-[9px] font-bold text-emerald-400' : 'text-[9px] font-bold text-rose-400'}>{changePct >= 0 ? '+' : ''}{changePct.toFixed(1)}%</span>}
      </div>
      <svg viewBox="0 0 320 104" className="mt-1 h-[104px] w-full" role="img" aria-label={`${normalized} one year daily candlestick mini chart`} preserveAspectRatio="none">
