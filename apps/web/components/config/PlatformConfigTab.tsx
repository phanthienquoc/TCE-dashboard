...
export default function PlatformConfigTab() {
  ...
  async function importSsiJson(file: File) {
    try {
      ...
      const imported: SsiCredentials = {
        apiKey: pick('apiKey', 'apikey', 'API Key'),
        apiSecret: pick('apiSecret', 'apisecret', 'secret', 'API Secret'),
        privateKey: pick('privateKey', 'privatekey', 'Private Key'),
      };
      if (!imported.apiKey || !imported.apiSecret) throw new Error('JSON must contain API Key and API Secret');
      setSsi(imported);
      setSsiAccountNo(pick('accountNo', 'accountNumber', 'Account No'));
  ...
  async function run(key: string, action: () => Promise<any>, success: string) {
    ...
    catch (e: any) { setStatus(s => ({ ...s, [key]: readError(e) })); }
  ...
  async function testSsi(withTradingAuth = false) {
    ...
    try {
      ...
      let message = 'API connection OK — verify trading access to load accounts';
      if (withTradingAuth) message = accounts.length ? `Trading OK — ${accounts.length} account(s)` : 'Trading authenticated — no account returned';
  ...
  return <div className="space-y-4">
    ...
    <Tabs defaultValue="ssi" className="w-full">
      ...
      <TabsContent value="ssi"><Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>SSI FastConnect</CardTitle><CardDescription>Core credentials only. JSON import is local.</CardDescription></div><StatusBadge value={status.ssi}/></div></CardHeader><CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2"><Field label="Environment"><select value={ssiEnv} onChange={e => setSsiEnv(e.target.value as 'production' | 'sandbox')} className="h-11 w-full rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white"><option value="production">Production</option><option value="sandbox">Sandbox</option></select></Field><Field label="JSON config"><div className="flex h-11 items-center gap-2"><input ref={fileRef} type="file" accept="application/json,.json" className="sr-only" onChange={e => { const file = e.target.files?.[0]; if (file) void importSsiJson(file); }} /><Button type="button" variant="outline" disabled={!!busy} onClick={() => fileRef.current?.click()}><Upload className="size-4"/>Upload JSON</Button>{jsonName && <span className="min-w-0 truncate text-xs text-zinc-400"><FileJson className="mr-1 inline size-3.5"/>{jsonName}</span>}</div></Field></div>
        <div className="grid gap-3 sm:grid-cols-2"><Field label="API Key"><Input value={ssi.apiKey} onChange={e => { setSsi({ ...ssi, apiKey: e.target.value }); resetSsiTest(); }} type="password" autoComplete="new-password" /></Field><Field label="API Secret"><Input value={ssi.apiSecret} onChange={e => { setSsi({ ...ssi, apiSecret: e.target.value }); resetSsiTest(); }} type="password" autoComplete="new-password" /></Field><Field label="Private Key"><textarea value={ssi.privateKey} onChange={e => { setSsi({ ...ssi, privateKey: e.target.value }); resetSsiTest(); }} rows={4} className="w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white" autoComplete="off" /></Field></div>
        {ssiTested && <div className="space-y-3 rounded-xl border border-white/10 bg-white/[0.025] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-medium">Trading access</span><span className="text-xs text-zinc-500">OTP is required by SSI for account/trading APIs.</span></div><div className="flex flex-wrap gap-2"><Input className="w-40" value={ssiOtp} onChange={e => setSsiOtp(e.target.value)} inputMode="numeric" autoComplete="one-time-code" placeholder="OTP" /><Button type="button" variant="outline" disabled={!!busy} onClick={() => void requestSsiOtp()}>Request OTP</Button><Button type="button" variant="outline" disabled={!!busy || (!ssiOtp && !ssiTransactionId)} onClick={() => void testSsi(true)}><Wifi className="size-4"/>Verify trading</Button></div>{ssiAccounts.length > 0 && <div className="flex flex-wrap items-center gap-2"><span className="text-xs text-zinc-400">Account</span><select value={ssiAccountNo} onChange={e => setSsiAccountNo(e.target.value)} className="h-10 min-w-48 rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white">{ssiAccounts.map(account => <option key={account.accountNo} value={account.accountNo}>{account.accountNo} · {account.accountType}</option>)}</select><Button type="button" disabled={!!busy || !ssiTradingVerified || !ssiAccountNo} onClick={() => void saveTestedSsi()}><Save className="size-4"/>Save</Button></div>}</div>}
      ...
      <TabsContent value="binance"><Card><CardHeader><CardTitle>Binance Futures</CardTitle><CardDescription>Environment is shared by save and connection test.</CardDescription></CardHeader><CardContent className="space-y-4"><Field label="Environment"><select value={binanceEnv} onChange={e => setBinanceEnv(e.target.value as 'production' | 'testnet')} className="h-11 w-full rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white"><option value="testnet">Testnet</option><option value="production">Production</option></select></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="API Key"><Input value={binance.apiKey} onChange={e => setBinance({ ...binance, apiKey: e.target.value })} type="password" /></Field><Field label="API Secret"><Input value={binance.apiSecret} onChange={e => setBinance({ ...binance, apiSecret: e.target.value })} type="password" /></Field></div><div className="flex flex-wrap gap-2"><Button type="button" disabled={!!busy} onClick={() => void run('binance', () => platformApi.save('binance', binanceEnv, binance), 'Saved')}><Save className="size-4"/>Save</Button><Button type="button" variant="outline" disabled={!!busy} onClick={() => void run('binance', () => platformApi.binanceTest(binanceEnv), 'Connection OK')}><Wifi className="size-4"/>Test connection</Button></div></CardContent></Card></TabsContent>
      <TabsContent value="fastapi"><Card><CardHeader><CardTitle>FastAPI</CardTitle><CardDescription>Backend service connection metadata.</CardDescription></CardHeader><CardContent className="space-y-4"><Field label="Base URL"><Input value={fastapi.baseUrl} onChange={e => setFastapi({ ...fastapi, baseUrl: e.target.value })} placeholder="https://api.example.com" /></Field><Field label="Health path"><Input value={fastapi.healthPath} onChange={e => setFastapi({ ...fastapi, healthPath: e.target.value })} placeholder="/health" /></Field><Button type="button" disabled={!!busy} onClick={() => void run('fastapi', () => platformApi.saveFastApi(fastapi), 'Saved')}><Save className="size-4"/>Save configuration</Button></CardContent></Card></TabsContent>
    </Tabs>
  </div>;
}

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div className="space-y-1.5"><label className="text-xs font-medium text-zinc-300">{label}</label>{children}</div>; }
