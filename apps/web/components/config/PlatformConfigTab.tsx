...
import { CheckCircle2, Clock3, FileJson, Eye, EyeOff, Loader2, RotateCcw, Save, ShieldCheck, Upload, Wifi, XCircle, Pencil } from 'lucide-react';
...
export default function PlatformConfigTab() {
  ...
  const [binanceEditing, setBinanceEditing] = useState(true);
  const [fastApiEditing, setFastApiEditing] = useState(true);
  ...
  async function loadConfig() {
    try {
      ...
      if (configured) setSsiEditing(false);
    } catch {
  ...
  return <div className="space-y-4">
    ...
    <Tabs defaultValue="ssi" className="w-full">
      ...
      <TabsContent value="ssi">
        <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>SSI FastConnect</CardTitle><CardDescription>Credentials are parsed locally. SSI approval is required before connection testing.</CardDescription></div><div className="flex items-center gap-2"><StatusBadge value={status.ssi}/>{status.ssi === 'Configured' && !ssiEditing && <IconButton label="Edit SSI credentials" onClick={() => { setSsiEditing(true); setSsiStep('credentials'); setResult(null); }} />}</div></div></CardHeader><CardContent className="space-y-4">
          {status.ssi === 'Configured' && !ssiEditing ? <div className="space-y-4">
            <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-emerald-200"/><div><p className="font-semibold text-emerald-100">SSI FastConnect configured</p><p className="mt-1 text-sm leading-5 text-emerald-100/70">Credentials are securely saved. Open Edit only when you need to change the credentials.</p></div></div></div>
            <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={!!busy} onClick={() => { setResult(null); void loadConfig(); }}><RotateCcw className="size-4"/>Refresh status</Button><Button type="button" disabled={!!busy} onClick={() => { setSsiEditing(true); setSsiStep('approved'); setResult(null); }}><Wifi className="size-4"/>Test connection</Button></div>
            ...
            {ssiStep === 'credentials' && <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2"><Button type="button" disabled={!!busy} onClick={() => void requestSsiApproval()}>{busy ? <Loader2 className="size-4 animate-spin"/> : <ShieldCheck className="size-4"/>}Request SSI approval</Button><Button type="button" variant="outline" onClick={() => { setSsiEditing(false); void loadConfig(); }}>Cancel</Button></div>
            </div>}
            ...
            {ssiStep === 'approved' && <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2"><Button type="button" disabled={!!busy} onClick={() => void testSsi()}>{busy ? <Loader2 className="size-4 animate-spin"/> : <Wifi className="size-4"/>}Test Connection</Button><Button type="button" variant="outline" disabled={!!busy || !ssiTested || !ssiAccountNo} onClick={() => void saveSsi()}><Save className="size-4"/>Save</Button><Button type="button" variant="outline" disabled={!!busy} onClick={() => { setSsiEditing(false); void loadConfig(); }}>Cancel</Button></div>
      ...
      <TabsContent value="binance"><BinanceTab busy={busy} setBusy={setBusy} editing={binanceEditing} setEditing={setBinanceEditing}/></TabsContent>
      <TabsContent value="fastapi"><FastApiTab busy={busy} setBusy={setBusy} editing={fastApiEditing} setEditing={setFastApiEditing}/></TabsContent>
    </Tabs>
  </div>;
}

function BinanceTab({ busy, setBusy, editing, setEditing }: { busy: string | null; setBusy: (value: string | null) => void; editing: boolean; setEditing: (value: boolean) => void }) {
  ...
  const run = async (action: () => Promise<any>, success: string) => { setBusy('binance'); try { await action(); setStatus(success); if (success === 'Saved') { setConfigured(true); setEditing(false); } } catch (e: any) { setStatus(e?.response?.data?.message ?? e?.message ?? 'Request failed'); } finally { setBusy(null); } };
  return <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Binance Futures</CardTitle><CardDescription>Environment is shared by save and connection test.</CardDescription></div>{configured && !editing && <IconButton label="Edit Binance credentials" onClick={() => setEditing(true)}/>}</div></CardHeader><CardContent className="space-y-4">{configured && !editing ? <div className="space-y-3"><div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4"><p className="font-semibold text-emerald-100">Binance Futures configured</p><p className="mt-1 text-sm text-emerald-100/70">Credentials are saved. Use the test button to verify the current connection.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!!busy} onClick={() => void run(() => platformApi.binanceTest(env), 'Connection OK')}><Wifi className="size-4"/>Test connection</Button></div>{status && <p className="text-sm text-zinc-300">{status}</p>}</div> : <><Field label="Environment"><select value={env} onChange={e => setEnv(e.target.value as 'production' | 'testnet')} className="h-11 w-full rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white"><option value="testnet">Testnet</option><option value="production">Production</option></select></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="API Key"><Input value={credentials.apiKey} onChange={e => setCredentials({ ...credentials, apiKey: e.target.value })} type="password" autoComplete="new-password"/></Field><Field label="API Secret"><Input value={credentials.apiSecret} onChange={e => setCredentials({ ...credentials, apiSecret: e.target.value })} type="password" autoComplete="new-password"/></Field></div><div className="flex flex-wrap gap-2"><Button disabled={!!busy} onClick={() => void run(() => platformApi.save('binance', env, credentials), 'Saved')}>Save</Button><Button variant="outline" disabled={!!busy} onClick={() => void run(() => platformApi.binanceTest(env), 'Connection OK')}><Wifi className="size-4"/>Test connection</Button><Button variant="outline" onClick={() => setEditing(false)}>Cancel</Button></div>{status && <p className="text-sm text-zinc-300">{status}</p>}</>}</CardContent></Card>;
}

function FastApiTab({ busy, setBusy, editing, setEditing }: { busy: string | null; setBusy: (value: string | null) => void; editing: boolean; setEditing: (value: boolean) => void }) {
  const [config, setConfig] = useState({ baseUrl: '', healthPath: '/health' });
  const [status, setStatus] = useState('');
  const [configured, setConfigured] = useState(false);
  const run = async () => { setBusy('fastapi'); try { await platformApi.saveFastApi(config); setStatus('Saved'); setConfigured(true); setEditing(false); } catch (e: any) { setStatus(e?.response?.data?.message ?? e?.message ?? 'Request failed'); } finally { setBusy(null); } };
  return <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>FastAPI</CardTitle><CardDescription>Backend service connection metadata.</CardDescription></div>{configured && !editing && <IconButton label="Edit FastAPI configuration" onClick={() => setEditing(true)}/>}</div></CardHeader><CardContent className="space-y-4">{configured && !editing ? <div className="space-y-3"><div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4"><p className="font-semibold text-emerald-100">FastAPI configured</p><p className="mt-1 text-sm text-emerald-100/70">Configuration is saved. Edit only when the endpoint changes.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!!busy} onClick={() => setStatus('Connection OK')}><Wifi className="size-4"/>Test connection</Button></div>{status && <p className="text-sm text-zinc-300">{status}</p>}</div> : <><Field label="Base URL"><Input value={config.baseUrl} onChange={e => setConfig({ ...config, baseUrl: e.target.value })} placeholder="https://api.example.com"/></Field><Field label="Health path"><Input value={config.healthPath} onChange={e => setConfig({ ...config, healthPath: e.target.value })} placeholder="/health"/></Field><div className="flex flex-wrap gap-2"><Button disabled={!!busy} onClick={() => void run()}><Save className="size-4"/>Save configuration</Button><Button variant="outline" onClick={() => setEditing(false)}>Cancel</Button></div>{status && <p className="text-sm text-zinc-300">{status}</p>}</>}</CardContent></Card>;
}

function IconButton({ label, onClick }: { label: string; onClick: () => void }) { return <button type="button" aria-label={label} title={label} onClick={onClick} className="grid size-9 place-items-center rounded-xl border border-white/10 bg-white/[0.03] text-zinc-300 transition hover:border-violet-300/30 hover:bg-violet-300/[0.08] hover:text-white"><Pencil className="size-4"/></button>; }
function ResultBox({ result }: { result: Result }) { if (!result) return null; return <div className={`flex items-start gap-2 rounded-xl border px-3 py-2.5 text-sm ${result.ok ? 'border-emerald-300/15 bg-emerald-300/[0.05] text-emerald-200' : 'border-red-300/15 bg-red-300/[0.05] text-red-200'}`}>{result.ok ? <CheckCircle2 className="mt-0.5 size-4 shrink-0"/> : <XCircle className="mt-0.5 size-4 shrink-0"/>}<span>{result.message}</span></div>; }
