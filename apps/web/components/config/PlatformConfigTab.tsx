...
import { CheckCircle2, ChevronDown, Loader2, Save, ShieldCheck, Wifi } from 'lucide-react';
...
type Environment = 'production' | 'sandbox' | 'testnet';
const field = 'space-y-1.5';

export default function PlatformConfigTab() {
  const [ssiEnv, setSsiEnv] = useState<'production' | 'sandbox'>('production');
  const [ssi, setSsi] = useState({ clientId:'', apiKey:'', apiSecret:'', accountNo:'', privateKey:'' });
  ...
  async function loadConfig() {
    try { const [credentials, fast] = await Promise.all([platformApi.credentials(), platformApi.fastApiConfig()]);
  ...
  async function run(key:string, action:()=>Promise<any>, success:string) { setBusy(key); setStatus(s=>({...s,[key]:'Working…'})); try { await action(); setStatus(s=>({...s,[key]:success})); } catch(e:any) { setStatus(s=>({...s,[key]:e?.response?.data?.message ?? 'Request failed'})); } finally { setBusy(null); } }

  return <div className="space-y-4">
    <div><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#887b91]">Platform configuration</p><h2 className="mt-1 text-xl font-semibold">Connections & environments</h2><p className="mt-1 text-sm text-zinc-400">Secrets are sent to the backend and are never rendered back after save.</p></div>
    <Tabs defaultValue="ssi" className="w-full">
      ...
      <TabsContent value="ssi">
        <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>SSI FastConnect</CardTitle><CardDescription>Connection credentials and environment.</CardDescription></div><StatusBadge value={status.ssi}/></div></CardHeader><CardContent className="space-y-4">
          <SelectField label="Environment" value={ssiEnv} onChange={v=>setSsiEnv(v as any)} options={[['production','Production'],['sandbox','Sandbox']]} />
          ...
          {ssiOpen && <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Account No."><Input value={ssi.accountNo} onChange={e=>setSsi({...ssi,accountNo:e.target.value})} autoComplete="off" /></Field>
            ...
            <Field label="Private Key" className="sm:col-span-2"><textarea value={ssi.privateKey} onChange={e=>setSsi({...ssi,privateKey:e.target.value})} rows={4} className="w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white outline-none focus:ring-2 focus:ring-violet-400/60" autoComplete="off" /></Field>
          </div>}
      ...
      <TabsContent value="binance">
        <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Binance Futures</CardTitle><CardDescription>Environment is shared by save and connection test.</CardDescription></div><StatusBadge value={status.binance}/></div></CardHeader><CardContent className="space-y-4">
          <SelectField label="Environment" value={binanceEnv} onChange={v=>setBinanceEnv(v as any)} options={[['testnet','Testnet'],['production','Production']]} />
