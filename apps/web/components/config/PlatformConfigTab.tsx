...
export default function PlatformConfigTab() {
  ...
  async function loadConfig() {
    try {
      ...
      const configured =
        Array.isArray(rows) &&
        rows.some((x: any) => x.provider === 'ssi' && (!x.environment || x.environment === ssiEnv));
  ...
  async function importSsiJson(file: File) {
    try {
      ...
      collect(parsed);
      collect((parsed as any)?.credentials);
      collect((parsed as any)?.ssi);
      collect((parsed as any)?.config);
      const normalized = (key: string) => key.toLowerCase().replace(/[^a-z0-9]/g, '');
      const pick = (...keys: string[]) => {
        const wanted = keys.map(normalized);
        for (const source of sources)
          for (const [key, value] of Object.entries(source))
            if (value != null && wanted.includes(normalized(key))) return String(value);
        return '';
      };
      ...
      if (!imported.apiKey || !imported.apiSecret)
        throw new Error('JSON must contain API Key and API Secret');
      setSsi(imported);
  ...
  async function checkApproval(silent = false) {
    ...
    try {
      const response = await platformApi.ssiApprove({
        environment: ssiEnv,
        credentials: ssi,
        transactionId: ssiTransactionId.trim() || undefined,
        otp: ssiOtp.trim() || undefined,
      });
      ...
      if (!silent)
        setResult({ ok: false, message: `Waiting for SSI approval: ${errorMessage(error)}` });
      return false;
    } finally {
      setApprovalChecking(false);
    }
  ...
  async function testSsi() {
    ...
    try {
      ...
      setSsiAccountNo(current =>
        current && accounts.some(a => a.accountNo === current)
          ? current
  ...
  async function saveSsi() {
    if (!ssiTested || !ssiAccountNo) {
      setResult({
        ok: false,
        message: 'Test Connection successfully and select an account before saving.',
      });
      return;
    }
    setBusy('ssi');
    try {
      const response = await platformApi.ssiSaveTested({
        environment: ssiEnv,
        credentials: ssi,
        accountNo: ssiAccountNo,
        transactionId: ssiTransactionId || undefined,
        otp: ssiOtp.trim() || undefined,
      });
      if (!response.data?.ok)
        throw new Error(response.data?.error?.message ?? 'Unable to save SSI credentials');
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">
          Configure credentials, approve the SSI login, then test and save the verified connection.
        </p>
      </div>
      <Tabs defaultValue="ssi" className="w-full">
        <TabsList className="w-full justify-start overflow-x-auto">
          <TabsTrigger value="ssi">SSI FastConnect</TabsTrigger>
          <TabsTrigger value="binance">Binance Futures</TabsTrigger>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <CardTitle>SSI FastConnect</CardTitle>
                  <CardDescription>
                    Credentials are parsed locally. SSI approval is required before connection
                    testing.
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <StatusBadge value={status.ssi} />
                  {status.ssi === 'Configured' && !ssiEditing && (
                    <IconButton
                      label="Edit SSI credentials"
                      onClick={() => {
                        setSsiEditing(true);
                        setSsiStep('credentials');
                        setResult(null);
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-emerald-100">SSI FastConnect configured</p>
                        <p className="mt-1 text-sm leading-5 text-emerald-100/70">
                          Credentials are securely saved. Open Edit only when you need to change the
                          credentials.
                        </p>
                      </div>
                    </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-2">
                    <Step
                      number="1"
                      title="Credentials"
                      active={ssiStep === 'credentials'}
                      done={ssiStep !== 'credentials'}
                    />
                    <Step
                      number="2"
                      title="SSI approval"
                      active={ssiStep !== 'credentials'}
                      done={ssiStep === 'approved'}
                    />
                  </div>
                  {ssiStep === 'credentials' && (
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <input
                          ...
                          onChange={e => {
                            ...
                            if (file) void importSsiJson(file);
                          }}
                        />
                        <Button
                          type="button"
                        ...
                        {jsonName && (
                          <span className="min-w-0 truncate text-xs text-zinc-400">
                            <FileJson className="mr-1 inline size-3.5" />
                            {jsonName}
                          </span>
                        )}
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        <Field label="Environment">
                          <select
                            value={ssiEnv}
                            onChange={e => {
                              setSsiEnv(e.target.value as 'production' | 'sandbox');
                              resetSsiFlow();
                            }}
                            className="h-11 w-full rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white outline-none focus:ring-2 focus:ring-violet-400/60"
                          >
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <Field label="API Key">
                          <Input
                            value={ssi.apiKey}
                            onChange={e => updateSsi('apiKey', e.target.value)}
                            type="password"
                        ...
                        <Field label="API Secret">
                          <Input
                            value={ssi.apiSecret}
                            onChange={e => updateSsi('apiSecret', e.target.value)}
                            type="password"
                            autoComplete="new-password"
                          />
                        </Field>
                        <Field label="Private Key">
                          <div className="relative">
                            <textarea
                              value={ssi.privateKey}
                              onChange={e => updateSsi('privateKey', e.target.value)}
                              rows={5}
                              className={`w-full rounded-xl border border-white/10 bg-black/20 p-3 pr-12 text-sm text-white outline-none focus:ring-2 focus:ring-violet-400/60 ${privateKeyVisible ? '' : 'tracking-[.18em]'}`}
                              autoComplete="off"
                              style={
                                privateKeyVisible
                                  ? undefined
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button
                          type="button"
                          disabled={!!busy}
                          onClick={() => void requestSsiApproval()}
                        >
                          {busy ? (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm leading-5 text-amber-100/70">
                              Open SSI iBoard/app and approve the login request. TCE checks the
                              approval automatically every 5 seconds.
                            </p>
                            <p className="mt-3 break-all text-[11px] text-amber-100/50">
                              Transaction ID: {ssiTransactionId}
                            </p>
                          </div>
                        </div>
                      </div>
                      <Field label="OTP (only if SSI asks for OTP)">
                        <Input
                          value={ssiOtp}
                          onChange={e => setSsiOtp(e.target.value)}
                          inputMode="numeric"
                          autoComplete="one-time-code"
                          placeholder="Enter SSI OTP"
                        />
                      </Field>
                      <div className="flex flex-wrap gap-2">
                        <Button
                  ...
                  {ssiStep === 'approved' && (
                    <div className="space-y-4">
                      <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm leading-5 text-emerald-100/70">
                              SSI accepted the login request. Test Connection is now enabled.
                            </p>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <Button type="button" disabled={!!busy} onClick={() => void testSsi()}>
                          {busy ? (
                            <Loader2 className="size-4 animate-spin" />
                          ) : (
                            <Wifi className="size-4" />
                        ...
                        <Button
                          type="button"
                          variant="outline"
                          disabled={!!busy || !ssiTested || !ssiAccountNo}
                          onClick={() => void saveSsi()}
                        >
                      ...
                      {ssiAccounts.length > 0 && (
                        <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="text-xs text-zinc-400">Account</span>
                            <select
                              value={ssiAccountNo}
                              onChange={e => setSsiAccountNo(e.target.value)}
                              className="h-10 min-w-48 rounded-xl border border-white/10 bg-[#120b18] px-3 text-sm text-white outline-none"
                            >
                              <option value="">Select account</option>
                              {ssiAccounts.map(a => (
                                <option key={a.accountNo} value={a.accountNo}>
                                  {a.accountNo} · {a.accountType}
                                </option>
                              ))}
                            </select>
                          </div>
                  ...
                  {result && <ResultBox result={result} />}
                  <p className="text-[11px] leading-5 text-[#75697d]">
                    JSON is parsed locally. Credentials are sent to the backend only for SSI
                    authentication, approval verification, connection testing, and saving.
                  </p>
                </>
              )}
            </CardContent>
        ...
        <TabsContent value="binance">
          <BinanceTab
            busy={busy}
            setBusy={setBusy}
            editing={binanceEditing}
            setEditing={setBinanceEditing}
          />
        </TabsContent>
        <TabsContent value="fastapi">
          <FastApiTab
            busy={busy}
            setBusy={setBusy}
            editing={fastApiEditing}
            setEditing={setFastApiEditing}
          />
        </TabsContent>
      </Tabs>
    </div>
...
function BinanceTab({
  ...
  const run = async (action: () => Promise<any>, success: string) => {
    ...
    try {
      ...
      setStatus(e?.response?.data?.message ?? e?.message ?? 'Request failed');
    } finally {
      setBusy(null);
    }
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-3">
          ...
          {configured && !editing && (
            <IconButton label="Edit Binance credentials" onClick={() => setEditing(true)} />
          )}
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        {configured && !editing ? (
          <div className="space-y-3">
            <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4">
              <p className="font-semibold text-emerald-100">Binance Futures configured</p>
              <p className="mt-1 text-sm text-emerald-100/70">
                Credentials are saved. Use the test button to verify the current connection.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                disabled={!!busy}
                onClick={() =>
                  void run(() => platformApi.save('binance', env, credentials), 'Saved')
                }
              >
                Save
              </Button>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-3">
          ...
          {configured && !editing && (
            <IconButton label="Edit FastAPI configuration" onClick={() => setEditing(true)} />
          )}
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        {configured && !editing ? (
          <div className="space-y-3">
            <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-4">
              <p className="font-semibold text-emerald-100">FastAPI configured</p>
              <p className="mt-1 text-sm text-emerald-100/70">
                Configuration is saved. Edit only when the endpoint changes.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                disabled={!!busy}
                onClick={() => setStatus('Connection OK')}
              >
                <Wifi className="size-4" />
                Test connection
              </Button>
...
function IconButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      ...
      title={label}
      onClick={onClick}
      className="grid size-9 place-items-center rounded-xl border border-white/10 bg-white/[0.03] text-zinc-300 transition hover:border-violet-300/30 hover:bg-violet-300/[0.08] hover:text-white"
    >
...
function ResultBox({ result }: { result: Result }) {
  ...
  return (
    <div
      className={`flex items-start gap-2 rounded-xl border px-3 py-2.5 text-sm ${result.ok ? 'border-emerald-300/15 bg-emerald-300/[0.05] text-emerald-200' : 'border-red-300/15 bg-red-300/[0.05] text-red-200'}`}
    >
      {result.ok ? (
...
function Step({
  ...
  return (
    <div
      className={`flex min-h-11 items-center gap-2 rounded-xl border px-3 ${done ? 'border-emerald-300/15 bg-emerald-300/[0.05]' : active ? 'border-violet-300/20 bg-violet-300/[0.06]' : 'border-white/10'}`}
    >
      {done ? (
        <CheckCircle2 className="size-4 text-emerald-200" />
      ) : (
        <span className="grid size-5 place-items-center rounded-full bg-violet-300/10 text-[10px] font-bold">
          {number}
        </span>
      )}
...
function StatusBadge({ value }: { value?: string }) {
  ...
  const ok = /configured|saved|ok|verified|confirmed/i.test(value);
  return (
    <span
      className={`rounded-full border px-2.5 py-1 text-[11px] ${ok ? 'border-emerald-400/20 text-emerald-300' : 'border-white/10 text-zinc-400'}`}
    >
      {value}
    </span>
  );
