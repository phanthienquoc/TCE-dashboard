  ...
  return (
    <div className="min-w-0 space-y-4 overflow-hidden">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm leading-6 text-muted">Configure each platform independently. Authentication and persistence stay inside its renderer.</p>
      </div>
      <Tabs defaultValue={platforms[0].id} className="w-full min-w-0">
        ...
        {platforms.map(platform => { const Component = platform.component; return <TabsContent key={platform.id} value={platform.id} className="min-w-0"><Component {...props} /></TabsContent>; })}
      </Tabs>
    </div>
  );
}
