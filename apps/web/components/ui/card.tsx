...
export const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('rounded-xl border border-border bg-surface-strong text-foreground shadow-[0_8px_28px_rgba(0,0,0,0.06)]', className)}
      {...props}
    />
  )
);
...
export const CardContent = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('p-4 pt-0 sm:p-5 sm:pt-0 text-foreground', className)} {...props} />
);
