...
export function EngineRuntimeSkeleton() {
  return <div className="tce-skeleton-dark" role="status" aria-label="Loading engine runtime">{Array.from({ length: 4 }, (_, index) => <div key={index} className="tce-skeleton-engine-row"><Skeleton className="size-10 rounded-xl" /><div className="tce-skeleton-copy"><Skeleton className="h-4 w-36" /><Skeleton className="mt-2 h-3 w-48 max-w-full" /></div><Skeleton className="h-8 w-14 rounded-full" /></div>)}<SkeletonStyles dark /></div>;
}
