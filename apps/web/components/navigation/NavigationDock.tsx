...
export function NavigationDock({ items, onSelect }: NavigationDockProps) {
  ...
  const isActive = (item: NavigationItem) => {
    ...
    try {
      ...
      return true;
    } catch { return Boolean(item.active); }
  };
  ...
  const renderItem = (item: NavigationItem) => {
    ...
    const content = <><Icon className="size-[19px] shrink-0 transition-transform duration-200" /><span className="truncate">{item.label}</span></>;
    if (item.href) return <Link key={item.id} href={item.href} prefetch aria-current={active ? 'page' : undefined} aria-label={item.label} onClick={() => setOpen(false)} className={className}>{content}</Link>;
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-[70] border-b border-border bg-surface-strong/95 px-[max(12px,env(safe-area-inset-left))] pt-[max(6px,env(safe-area-inset-top))] pr-[max(12px,env(safe-area-inset-right))] shadow-sm backdrop-blur-xl">
      <div className="mx-auto flex h-12 w-full items-center justify-between">
        <Link href="/dashboard" aria-label="TCE Dashboard" className="flex min-w-0 items-center gap-2 rounded-xl py-1.5 pr-3 transition-transform duration-200 active:scale-[.98]"><span className="grid size-9 shrink-0 place-items-center rounded-[11px] bg-foreground text-[11px] font-bold tracking-[-.02em] text-background">TCE</span><span className="truncate text-[15px] font-semibold tracking-[-.02em] text-foreground">Treasury Cash Extraction</span></Link>
        <Button type="button" variant="ghost" size="icon" className="size-10 shrink-0 rounded-xl transition-transform duration-200 active:scale-90" onClick={() => setOpen(true)} aria-label="Open Services" aria-expanded={open}><Menu className="size-[22px]" /></Button>
      </div>
    </header>
    {open && <div className="fixed inset-0 z-[80] animate-in fade-in duration-200" role="dialog" aria-modal="true" aria-label="Services menu">
      ...
      <aside className="absolute right-0 top-0 h-full w-[min(86vw,340px)] animate-in slide-in-from-right duration-300 border-l border-border bg-surface-strong p-5 shadow-[-18px_0_50px_rgb(0_0_0/.16)]">
        <div className="flex items-center justify-between border-b border-border pb-4"><div><p className="eyebrow">TCE</p><p className="mt-1 text-xl font-semibold tracking-tight text-foreground">Services</p></div><Button type="button" variant="ghost" size="icon" className="size-10 rounded-full transition-transform duration-200 active:rotate-90" onClick={() => setOpen(false)} aria-label="Close Services"><X className="size-5" /></Button></div>
