...
const isStaticAsset = request => {
  ...
  return (
    ...
    (url.pathname.startsWith('/_next/static/') ||
      ...
      /\.(?:css|js|mjs|png|jpg|jpeg|webp|svg|ico|woff2?|ttf)$/i.test(url.pathname))
  );
};
  ...
  event.waitUntil(
    caches
      ...
      .then(keys =>
        Promise.all(
          keys
            .filter(key => key.startsWith('tce-static-') && key !== STATIC_CACHE)
...
self.addEventListener('fetch', event => {
  ...
  if (isApiRequest(url.pathname) || isAuthRequest(url.pathname)) {
      ...
      self.clients
        ...
        .then(clients =>
          clients.forEach(client =>
            client.postMessage({ type: data.type || 'SYSTEM_UPDATE', title, body, version, url })
          )
        ),
    ])
  );
