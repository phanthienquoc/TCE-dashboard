...
api.interceptors.response.use(r => r, async (error: AxiosError) => {
  ...
  const token = await refreshPromise; if (!token) throw error; config.headers.Authorization = `Bearer ${token}`; return api.request(config);
});
export const authApi = { status: () => api.get('/auth/status'), login: (email:string,password:string) => api.post('/auth/login',{email,password}), mfaLogin: (userId:string,code:string) => api.post('/auth/mfa/login',{userId,code}), recovery: (userId:string,code:string) => api.post('/auth/mfa/recovery',{userId,code}), me: () => api.get('/auth/me'), refresh: () => api.post('/auth/refresh'), logout: () => api.post('/auth/logout') };
export const dashboardApi = { all: (status?: string) => api.get('/dashboard', { params: status ? { status } : undefined }), account:()=>api.get('/dashboard/account'), positions:()=>api.get('/dashboard/positions'), orders:()=>api.get('/dashboard/orders'), pools:(status?: string)=>api.get('/dashboard/pools', { params: status ? { status } : undefined }), nextPositions:()=>api.get('/dashboard/next-positions'), strategy:()=>api.get('/dashboard/strategy'), sources:()=>api.get('/dashboard/sources'), engines:()=>api.get('/dashboard/engines'), setEngineStatus:(engineId:string,status:'ACTIVE'|'INACTIVE')=>api.patch(`/dashboard/engines/${engineId}/status`,{status}), engineConfig:()=>api.get('/dashboard/engine-config'), setEngineConfig:(config:Record<string,unknown>)=>api.patch('/dashboard/engine-config',{config}) };
