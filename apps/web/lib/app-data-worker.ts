...
async function runWithConcurrency(tasks: Task[], concurrency: number, onDone: (failed: boolean) => void) {
  let cursor = 0;
...
export const useAppDataWorkerStore = create<AppDataWorkerState>((set, get) => ({
  ...
  start: async () => {
    ...
    inFlight = (async () => {
      try {
        // Existing shared stores own the response data consumed by their pages.
...
export const startAppDataWorker = () => useAppDataWorkerStore.getState().start();
