type CredentialEnvelope = { v: 1; alg: 'RSA-OAEP-256+A256GCM'; key: string; iv: string; tag: string; data: string };
...
export async function encryptCredentialPayload(value: unknown): Promise<CredentialEnvelope> {
  ...
  const key = await crypto.subtle.importKey('spki', pemToArrayBuffer(PUBLIC_KEY), { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['encrypt']);
  const aes = await crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, true, ['encrypt']);
  const rawAes = new Uint8Array(await crypto.subtle.exportKey('raw', aes));
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const plaintext = new TextEncoder().encode(JSON.stringify(value));
  const encrypted = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aes, plaintext));
  ...
  return { v: 1, alg: 'RSA-OAEP-256+A256GCM', key: toBase64Url(wrappedKey), iv: toBase64Url(iv), tag: toBase64Url(tag), data: toBase64Url(data) };
}
