...
function adapter(orders = [
  ...
  return new TceSsiOrderReconciliationAdapter(ssi as never, supabase as never, authorization as never);
}
...
test('fails closed on account/environment authorization mismatch', async () => {
  ...
  const instance = new TceSsiOrderReconciliationAdapter(
    { current: async () => { throw new Error('must not read SSI'); } } as never,
    { db: { from: () => ({ select: () => ({ eq: () => ({ maybeSingle: async () => ({ data: { id: 'account-1', user_id: 'user-1' }, error: null }) }) }) }) } } as never,
