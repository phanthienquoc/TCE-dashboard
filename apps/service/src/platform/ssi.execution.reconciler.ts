...
export class SsiExecutionReconciler {
  ...
  async reconcile(accountId: string, session: { adapter: SsiBrokerAdapter; accountNo: string }, event: SsiOrderStatusEvent) {
    ...
    if (!TERMINAL.has(status)) return { ignored: false as const, status, filled: false, partial: false, positionSynced: false };
    if (!FILLED.has(status) && !PARTIAL.has(status) && filledQuantity <= 0) {
      return { ignored: false as const, status, filled: false, partial: false, positionSynced: false };
    }
