...
export class SsiAssetSyncService {
  ...
  async sync(userId: string, environment = 'production', input: SsiAuthInput = {}) {
    ...
    for (const snapshot of snapshots.data) {
      ...
      for (const position of snapshot.positions) {
        ...
        const normalized = {
          ...
          averagePrice: Math.max(0, Number(position.averagePrice ?? 0)),
        };
        ...
        const { error: assetError } = await this.db.db.from('tce_broker_assets').upsert(
          {
            ...
            pending_quantity:
              Math.max(0, Number(position.buyingQuantity ?? 0)) +
              Math.max(0, Number(position.sellingQuantity ?? 0)),
            t_plus_quantity:
              Math.max(0, Number(position.t1SellQuantity ?? 0)) +
              Math.max(0, Number(position.t2SellQuantity ?? 0)),
      ...
      const staleIds = (existingAssets ?? [])
        .filter(existing => {
          const code = String(existing.asset_code ?? '')
            .trim()
            .toUpperCase();
    ...
    for (const [symbol, position] of aggregate) {
      ...
      const { error: positionError } = await this.db.db.from('tce_positions').upsert(
        {
          account_id: account.id,
