...
export class BinanceFuturesService {
  ...
  private environment(value = 'production'): BinanceEnvironment { if (value !== 'production' && value !== 'testnet') throw new Error(`Unsupported Binance environment: ${value}`); return value; }
  private async adapter(userId: string, environment = 'production') { const selected = this.environment(environment); const credentials = await this.credentials.get(userId, 'binance', selected); return new BinanceFuturesExecutionAdapter({ apiKey: typeof credentials.apiKey === 'string' ? credentials.apiKey : undefined, apiSecret: typeof credentials.apiSecret === 'string' ? credentials.apiSecret : undefined }, selected); }
