...
export class SsiService {
  ...
  private createAuth(credentials: SsiCredentials) {
    return new Auth(new Config({
      clientId: credentials.clientId ?? '', apiKey: credentials.apiKey ?? '', apiSecret: credentials.apiSecret ?? '',
      privateKey: credentials.privateKey ?? '', apiUrl: SSI_BASE_URL, streamingUrl: SSI_STREAM_URL,
      timeout: 60000, maxRetries: 5, retryDelay: 2000, rateLimitPerSecond: 10,
    }));
  }

  private async authenticate(credentials: SsiCredentials, authInput: SsiAuthInput = {}) {
    const auth = this.createAuth(credentials);
    const token = authInput.transactionId ? await auth.authenticate(undefined, authInput.transactionId) : await auth.authenticate(authInput.otp);
    return { auth, token };
  }
  ...
  async requestOtp(userId: string, environment = 'production') {
    ...
    } catch (error) { throw this.ssiError('SSI OTP request failed', error); }
  }
  ...
  async test(userId: string, environment = 'production', authInput: SsiAuthInput = {}) {
    ...
    } catch (error) { throw this.ssiError('SSI authentication/market-data check failed', error); }
  }
  ...
  async syncPortfolio(userId: string, environment = 'production', authInput: SsiAuthInput = {}) {
    try {
      ...
      const positionRows = (positions ?? []).filter((p) => Number(p.quantity ?? 0) > 0).map((p) => {
        const quantity = Number(p.quantity ?? 0), avgCost = Number(p.costPrice ?? 0);
        return { account_id: userId, symbol: String(p.symbol).toUpperCase(), quantity: Math.trunc(quantity), avg_cost: avgCost, cost_basis: quantity * avgCost, market_price: null, market_value: null, unrealized_pnl: null, status: 'OPEN', cycle_no: 0, updated_at: new Date().toISOString() };
      });
      ...
      for (const order of orders ?? []) {
        const quantity = Number(order.filledQuantity ?? order.quantity ?? 0), price = Number(order.avgPrice ?? order.price ?? 0);
        if (!order.orderId || !order.symbol || quantity <= 0) continue;
        const sideRaw = String(order.side ?? '').toUpperCase();
        const side = sideRaw === 'B' ? 'BUY' : sideRaw === 'S' ? 'SELL' : sideRaw;
        const gross = quantity * price, status = String(order.status ?? 'UNKNOWN').toUpperCase();
        const note = `SSI order ${order.orderId}`;
        const row = { account_id: userId, order_date: String(order.inputTime ?? new Date().toISOString()).slice(0, 10), symbol: String(order.symbol).toUpperCase(), side, price, quantity: Math.trunc(quantity), gross_value: gross, fee_tax: 0, net_cashflow: side === 'BUY' ? -gross : side === 'SELL' ? gross : 0, cycle_no: 0, status, note };
        const existing = await this.supabase.db.from('tce_orders').select('id').eq('account_id', userId).eq('note', note).maybeSingle();
        if (existing.error) throw existing.error;
        const result = existing.data
          ? await this.supabase.db.from('tce_orders').update(row).eq('id', existing.data.id)
          : await this.supabase.db.from('tce_orders').insert(row);
        if (result.error) throw result.error;
        ordersSynced += 1;
      }

      return { ok: true, provider: 'ssi', apiVersion: 'v3', accountNo, positionsSynced: positionRows.length, ordersSynced, balance: { availableCash: Number(balance?.availableCash ?? 0), withdrawal: Number(balance?.withdrawal ?? 0), totalDebt: Number(balance?.totalDebt ?? 0) } };
    } catch (error) { throw this.ssiError('SSI portfolio sync failed', error); }
  }

  private ssiError(prefix: string, error: unknown) { return new ServiceUnavailableException(`${prefix}: ${error instanceof Error ? error.message : String(error)}`); }
}
