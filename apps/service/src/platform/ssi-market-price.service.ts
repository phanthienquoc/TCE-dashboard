...
export class SsiMarketPriceService implements OnModuleInit, OnModuleDestroy {
    ...
    if (!this.acquireLock(lockKey))
      return {
        ...
        error: {
          ...
          message: 'SSI market sync is already running for this user',
        },
      };
    try {
      ...
      if (result.symbolsRequested > 0 && result.symbolsSynced === 0)
        return {
          ok: false as const,
          error: {
            code: result.errors[0]?.code ?? 'NO_MARKET_PRICES',
            message: result.errors[0]?.message ?? 'SSI returned no market prices',
          },
  ...
  async syncSymbolsNow(userId: string, symbols: string[], chunkSize = 200) {
    ...
    try {
      ...
      for (let offset = 0; offset < normalized.length; offset += chunkSize) {
        ...
        if (!quotes.ok) {
          chunk.forEach(symbol => failedSymbols.add(symbol));
          errors.push({
            userId,
            code: quotes.error.code,
            message: quotes.error.message,
            symbols: chunk,
          });
          continue;
        }
        ...
        if (missing.length) {
          ...
          const fallback = await this.ssi.dailyCloses(userId, 'production', missing, tradingDate);
          if (fallback.ok)
            for (const close of fallback.data) {
              const symbol = close.symbol.toUpperCase();
              fallbackSymbols.add(symbol);
              await this.persistPrice(
                userId,
                symbol,
                close.closePrice,
                close.closePrice,
        ...
        const unresolved = missing.filter(symbol => !fallbackSymbols.has(symbol));
        unresolved.forEach(symbol => failedSymbols.add(symbol));
        if (unresolved.length)
          errors.push({
            userId,
            code: 'PARTIAL_MARKET_DATA',
            message: `SSI returned no usable market data for ${unresolved.length}/${chunk.length} requested symbols`,
            symbols: unresolved,
          });
      ...
      return {
        ok: symbolsSynced > 0 || normalized.length === 0,
        data: {
          symbolsRequested: normalized.length,
          symbolsSynced,
          failedSymbols: [...failedSymbols].sort(),
          errors,
          partial: symbolsSynced > 0 && symbolsSynced < normalized.length,
        },
        errors,
      } as const;
    } finally {
  ...
  private userLockKey(userId: string): SyncLockKey {
    return `user:${userId}`;
  }
  ...
  private releaseLock(key: SyncLockKey): void {
  ...
  private isWeekday(day: number) {
    return day >= 1 && day <= 5;
  }
  private tradingDate(p: ReturnType<SsiMarketPriceService['nowParts']>) {
    return `${p.year.toString().padStart(4, '0')}-${p.month.toString().padStart(2, '0')}-${p.day.toString().padStart(2, '0')}`;
  }
  private async tick() {
    if (this.runningLocks.has('scheduled')) return;
  ...
  private async usersAndSymbols(userId?: string) {
    ...
    for (const account of accounts ?? []) {
      ...
      const [{ data: positions, error: positionsError }, { data: pools, error: poolsError }] =
        await Promise.all([
          this.db.db
            .from('tce_positions')
            .select('symbol')
    ...
    return [...byUser.entries()]
      .filter(([, symbols]) => symbols.size)
      .map(([accountUserId, symbols]) => ({ userId: accountUserId, symbols: [...symbols] }));
  }
  ...
  private async syncHourlyPrices(userId?: string): Promise<MarketSyncResult> {
    ...
    for (const user of users) {
      ...
      if (missing.length) {
        const tradingDate = this.tradingDate(this.nowParts());
        const fallback = await this.ssi.dailyCloses(
          user.userId,
          'production',
          missing,
          tradingDate
      ...
      const unresolved = missing.filter(
        symbol => !fallbackSymbols.has(symbol) && !returnedSymbols.has(symbol)
      );
      unresolved.forEach(symbol => failedSymbols.add(symbol));
      if (unresolved.length)
        errors.push({
          userId: user.userId,
          code: 'PARTIAL_MARKET_DATA',
          message: `SSI returned no usable market data for ${unresolved.length}/${user.symbols.length} requested symbols`,
          symbols: unresolved,
        });
      for (const quote of quotes.data) {
        await this.persistPrice(
  ...
  private async syncDailyClose(tradingDate: string) {
    const users = await this.usersAndSymbols(),
      observedAt = new Date().toISOString();
    for (const user of users) {
      const closes = await this.ssi.dailyCloses(
        user.userId,
        'production',
        user.symbols,
        tradingDate
      ...
      if (!closes.ok) {
        console.error('[SSI_DAILY_CLOSE_USER]', user.userId, closes.error);
        continue;
      }
  ...
  private async persistPrice(
    ...
    const { error: historyError } = await this.db.db
      .from('tce_market_prices')
      .upsert(payload, { onConflict: 'user_id,symbol,trading_date' });
    if (historyError) throw historyError;
    const { data: positions, error: positionError } = await this.db.db
      .from('tce_positions')
    ...
    for (const position of positions ?? []) {
      ...
      const { error } = await this.db.db
        ...
        .update({
          ...
          unrealized_pnl: quantity * (price - avgCost),
          updated_at: observedAt,
        })
        .eq('id', position.id);
