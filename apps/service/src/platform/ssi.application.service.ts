...
export class SsiApplicationService {
  ...
  private async handleOrderEvent(accountId: string, session: { adapter: SsiBrokerAdapter; accountNo: string }, event: SsiOrderStatusEvent) {
    ...
    if (event.status === 'FF' || event.status === 'PF' || event.status === 'FFPC') { const positions = await session.adapter.positions(session.accountNo); if (positions.ok) for (const position of positions.data) await this.positions.upsert({ ...position, accountId }); }
  }

  private async startOrderStream(accountId: string, session: { adapter: SsiBrokerAdapter; accountNo: string }) { try { await session.adapter.startOrderStatusStream(session.accountNo, (event) => { void this.handleOrderEvent(accountId, session, event).catch((error) => console.error('[SSI_ORDER_EVENT]', error)); }); } catch (error) { console.error('[SSI_ORDER_STREAM_START]', error); } }
  ...
  async approve(userId: string, environment: string, input: SsiAuthInput, credentials: Record<string, unknown>) { const session = this.fromRaw(credentials, userId, environment, undefined, true); const result = await session.adapter.connect({ userId, environment, ...input }); if (!result.ok) return result; const token = session.adapter.getTokenSnapshot(); const finalSession = token ? this.fromRaw({ ...credentials, ...token }, userId, environment, undefined, true) : session; this.storeSession(userId, environment, finalSession); if (finalSession.accountNo) void this.startOrderStream(await this.tceAccountId(userId), finalSession); return { ok: true as const, data: { authentication: 'ok' as const, provider: 'ssi' as const } }; }
  async test(userId: string, environment: string, input: SsiAuthInput, credentials?: Record<string, unknown>) { const key = `${userId}:ssi:${environment}`; const existing = this.sessions.get(key); const session = credentials && !input.otp?.trim() && !input.transactionId?.trim() && existing ? existing : credentials ? this.fromRaw(credentials, userId, environment) : await this.adapter(userId, environment); if (!input.otp?.trim() && !input.transactionId?.trim() && !existing) { const challenge = await session.adapter.requestOtp(); if (!challenge.ok) return challenge; return { ok: false as const, error: { code: 'PROVIDER_ERROR' as const, message: 'SSI_AUTH_REQUIRED' as const, retryable: false, provider: 'ssi', details: { transactionId: challenge.data.transactionId, message: challenge.data.message, action: 'APPROVE_OR_ENTER_OTP' } } }; } const result = await session.adapter.test(input); if (!result.ok) return result; const token = session.adapter.getTokenSnapshot(); const testedCredentials = token ? { ...credentials, ...token } : credentials; const finalSession = credentials && !existing ? this.fromRaw(testedCredentials!, userId, environment, undefined, true) : session; this.storeSession(userId, environment, finalSession); if (finalSession.accountNo) void this.startOrderStream(await this.tceAccountId(userId), finalSession); return result; }
  async saveTested(userId: string, environment: string, credentials: Record<string, unknown>, input: SsiAuthInput, accountNo: string) { if (!accountNo) throw new NotFoundException('SSI account number is required'); const key = `${userId}:ssi:${environment}`; const existing = this.sessions.get(key); if (existing) { const token = existing.adapter.getTokenSnapshot(); const persistedCredentials = { ...credentials, accountNo, ...(token ?? {}) }; await this.credentials.save(userId, 'ssi', environment, persistedCredentials); const persisted = this.fromRaw(persistedCredentials, userId, environment, accountNo, true); this.storeSession(userId, environment, persisted); void this.startOrderStream(await this.tceAccountId(userId), persisted); return { ok: true as const, data: { saved: true as const, provider: 'ssi' as const } }; } const session = this.fromRaw(credentials, userId, environment, accountNo); const result = await session.adapter.test(input); if (!result.ok) return result; const token = session.adapter.getTokenSnapshot(); const persistedCredentials = { ...credentials, accountNo, ...(token ?? {}) }; await this.credentials.save(userId, 'ssi', environment, persistedCredentials); const persisted = this.fromRaw(persistedCredentials, userId, environment, accountNo, true); this.storeSession(userId, environment, persisted); void this.startOrderStream(await this.tceAccountId(userId), persisted); return result; }
  ...
  async accountSnapshots(userId: string, environment: string, input: SsiAuthInput) { const key = `${userId}:ssi:${environment}`; const existing = this.sessions.get(key); if (existing) return existing.adapter.accountSnapshots(input); const { adapter } = await this.adapter(userId, environment, false); return adapter.accountSnapshots(input); }
