...
export class AuthRepository {
  ...
  async findUserByEmail(email: string): Promise<UserRow|null> {
    const { data, error } = await this.supabase.db.from('users').select('*').eq('email', email.toLowerCase()).maybeSingle();
    if (error) throw error;
    return data as UserRow|null;
  }

  async findUserById(id: string): Promise<UserRow|null> {
    const { data, error } = await this.supabase.db.from('users').select('*').eq('id', id).maybeSingle();
    if (error) throw error;
    return data as UserRow|null;
  }
  ...
  async rotateRefreshToken(tokenHash: string, newTokenHash: string, expiresAt: Date, ip?: string, userAgent?: string) {
    ...
    return row as { user_id: string; role: 'USER'|'ADMIN'; new_session_id: string; reuse_detected: boolean };
  }

  async consumeRecoveryCode(userId: string, codeHash: string): Promise<boolean> {
    const { data, error } = await this.supabase.db.rpc('consume_recovery_code', { p_user_id: userId, p_code_hash: codeHash });
    if (error) throw error;
    return data === true;
  }
}
