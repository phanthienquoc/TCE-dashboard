...
export class AuthController {
 ...
 @Post('login') async login(@Body() body:{email:string;password:string},@Headers('x-forwarded-for') ip?:string,@Headers('user-agent') ua?:string){const email=body?.email?.trim().toLowerCase();if(!email||typeof body?.password!=='string')throw new UnauthorizedException('Invalid credentials');const user=await this.repo.findUserByEmail(email);if(!user||!(await this.passwords.verify(body.password,user.password_hash)))throw new UnauthorizedException('Invalid credentials');if(user.mfa_enabled)return {mfaRequired:true,userId:user.id};return {accessToken:this.jwt.issue(user.id,user.role),refreshToken:await this.auth.createRefreshSession(user.id,this.clientIp(ip),ua)};}
