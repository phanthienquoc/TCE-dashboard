...
export type TakeProfitDecision =
  ...
  | {
      eligible: false;
      reason: 'NOT_AVAILABLE' | 'TCE_NO_DECISION' | 'TP_NOT_REACHED' | 'INVALID_PRICE';
    };
...
export class TakeProfitService {
  ...
  async evaluateCurrent(
    ...
    if (!Number.isFinite(decision.takeProfitPrice) || decision.takeProfitPrice <= 0)
      return { eligible: false, reason: 'INVALID_PRICE' };
    ...
    if (marketPrice < decision.takeProfitPrice)
      return { eligible: false, reason: 'TP_NOT_REACHED' };
  ...
  async recordConfirmedSale(
    ...
    if (!['SELL_APPROVED', 'SELL_PENDING'].includes(position.state))
    ...
    if (!Number.isFinite(salePrice) || salePrice <= 0 || quantity <= 0 || entryPrice <= 0)
      return { eligible: false, reason: 'INVALID_PRICE' };
