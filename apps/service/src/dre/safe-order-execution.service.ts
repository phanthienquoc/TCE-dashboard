...
export class SafeOrderExecutionService {
  ...
  async submit(action: DreAction, runId: string, mode: 'DRY_RUN' | 'LIVE', executor?: DreExecutionPort): Promise<{ status: DreOrderStatus; externalId?: string }> {
    await this.repository.ensureIndexes();
    const existing = await this.repository.getAction(action.idempotencyKey);
    if (existing) {
      if (existing.status === 'UNKNOWN' || existing.status === 'SUBMITTED' || existing.status === 'PARTIAL') return { status: existing.status, externalId: existing.externalId };
      if (existing.status === 'FILLED') return { status: 'FILLED', externalId: existing.externalId };
    }
    if (mode === 'DRY_RUN') {
    ...
    try {
      ...
      await this.repository.saveAction({ ...pending, status, externalId: response.externalId, submittedAt: new Date().toISOString() });
      return { status, externalId: response.externalId };
    } catch (error) {
      await this.repository.saveAction({ ...pending, status: 'UNKNOWN', error: error instanceof Error ? error.message : String(error) });
  ...
  async reconcile(externalId: string, broker: BrokerStatePort): Promise<DreOrderStatus> {
    ...
    const record = (await this.repository.listPending()).find(item => item.externalId === externalId);
    if (record) await this.repository.saveAction({ ...record, status: state.status, filledQuantity: state.filledQuantity });
    return state.status;
  }
}

export const isSupportedActionType = (type: DreActionType): boolean => type === 'BUY' || type === 'SELL';
