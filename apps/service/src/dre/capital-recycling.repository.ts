...
export class CapitalRecyclingRepository {
  ...
  async ensureIndexes(): Promise<void> {
    ...
    await db.collection<RecycledCapitalDocument>(COLLECTION).createIndex(
      { positionId: 1 },
      { unique: true, name: 'dre_recycled_position_unique' },
    );
  }

  async record(entry: RecycledCapitalEntry): Promise<RecycledCapitalEntry> {
    const db = await this.mongo.getDb();
    await db.collection<RecycledCapitalDocument>(COLLECTION).replaceOne(
      { _id: entry.positionId },
      { ...entry, _id: entry.positionId },
      { upsert: true },
    );
    return entry;
  }
  ...
  async listForCampaign(campaignId: string): Promise<RecycledCapitalEntry[]> {
    const db = await this.mongo.getDb();
    return db.collection<RecycledCapitalDocument>(COLLECTION).find({ campaignId }).sort({ confirmedAt: 1 }).toArray();
  }
}
