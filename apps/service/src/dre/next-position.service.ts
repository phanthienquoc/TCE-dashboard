  ...
  async prepare(input: {
    ...
    if (current && !['SOLD', 'COMPLETED', 'MISSED'].includes(current.state)) {
      ...
      return {
        status: 'NEXT_PENDING',
        position,
        reason: 'BLOCKED_BY_SEQUENCE',
        settledCash: await this.cash.getSettledCash(input.accountId),
        requiredCash: 0,
      };
    }
    ...
    if (!Number.isFinite(decision.quantity) || decision.quantity <= 0)
      return {
        status: 'NEXT_PENDING',
        position,
        reason: 'INVALID_TCE_QUANTITY',
        settledCash,
        requiredCash: 0,
      };
    if (!Number.isFinite(decision.entryPrice) || decision.entryPrice <= 0)
      return {
        status: 'NEXT_PENDING',
        position,
        reason: 'INVALID_TCE_PRICE',
        settledCash,
        requiredCash: 0,
      };
    ...
    return {
      status: 'NEXT_POSITION_READY',
      position: saved ?? { ...position, tceDecision: decision },
      action,
      settledCash,
      requiredCash,
    };
