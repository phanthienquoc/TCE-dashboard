...
import { createDividendCampaign, type DividendCampaign, type DividendCampaignInput } from './dividend-campaign';
...
export class DividendCampaignService {
  ...
  async list(status?: DreCampaignStatus): Promise<DividendCampaign[]> {
    return (await this.repository.listCampaigns(status)).map(campaign => campaign as DividendCampaign);
  }

  async updateStatus(id: string, status: DreCampaignStatus): Promise<DividendCampaign | null> {
    return (await this.repository.updateCampaignStatus(id, status, new Date().toISOString())) as DividendCampaign | null;
  }
}
