...
describe('StockEventsSupabaseRepository', () => {
  it('maps the canonical row shape to the API event shape', async () => {
              ...
              order: jest.fn(() => ({
                limit: jest.fn().mockResolvedValue({
                  data: [{
                    ...
                    event_content: 'Dividend', ratio_text: '10%', dividend_value: 1000,
                    reference_price: 20000, crawled_at: null,
                  }], error: null,
                }),
              }),
            })),
    ...
    expect(result[0]).toMatchObject({ id: 'mongo-1', ticker: 'AAA', exDividendDate: '2026-01-02', dividendValue: 1000, price: 20000 });
  });
});
