...
export class VietstockEventsCrawler {
  ...
  private async createBrowserSession(fromDate: string, toDate: string): Promise<BrowserSession> {
    ...
    const requestListener = (request: import('puppeteer').HTTPRequest) => {
      if (
        request.method() === 'POST' &&
        request.url().includes(EVENTS_API) &&
        request.postData()
      ) {
    ...
    try {
      ...
      if (!requestBody) {
        const token = await page.evaluate(() =>
          ...
          document.querySelector<HTMLInputElement>('#__RequestVerificationToken')?.value ||
          '',
        );
        if (token) {
      ...
      if (!requestBody) {
        throw new Error('Vietstock events browser request not captured');
      }
  ...
  private async fetchPage(pageNumber: number, session: BrowserSession): Promise<VietstockPage> {
    const text = await session.page.evaluate(
      async ({ requestBody, page }) => {
        const params = new URLSearchParams(requestBody);
        params.set('page', String(page));
        const response = await fetch('/data/eventstransferdata', {
          method: 'POST',
          headers: {
            Accept: '*/*',
  ...
  private parseApiResponse(text: string): VietstockPage {
    ...
    const total = this.firstNumber(value, [
      'recordsFiltered',
      'RecordsFiltered',
      'total',
      'Total',
  ...
  private parseHtmlRows(html: string) {
    ...
    return matches.slice(1).map(row => {
      ...
      return Object.fromEntries(
        headers.map((header, index) => [header, this.clean(this.stripTags(cells[index] ?? ''))]),
      );
    });
  }
