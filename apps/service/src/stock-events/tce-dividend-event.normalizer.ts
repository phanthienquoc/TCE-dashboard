...
export function normalizeTceDividendEvent(
  ...
  const symbol = String(row['Mã CK'] ?? row.symbol ?? '').trim().toUpperCase();
...
function stableEventId(rawId: unknown, symbol: string, exDividendAt: string, rate?: string | null): string {
