...
export class TelegramBotService {
  ...
  constructor(@Inject(CONTRACT_TOKENS.credentials) private readonly credentials: PlatformCredentialPort, private readonly signals: TceSignalService) {}
  ...
  parseSignal(text: string): TceTelegramSignal {
    ...
    const signal: TceTelegramSignal = { symbol: head[1].toUpperCase(), side: head[2].toUpperCase() as 'BUY' | 'SELL', entry: value(entry[0]), tp: value(tp[0]), sl: value(sl[0]) };
  ...
  async configure(userId: string, token: string, chatId?: string, environment = 'production') {
    ...
    const saved = await this.credentials.save(userId, 'telegram', environment, { botToken: token.trim(), chatId: chatId?.trim() || undefined });
  ...
  async start(userId: string, environment = 'production', token?: string, chatId?: string) {
    const key = `${userId}:${environment}`; if (this.running.has(key)) return;
    let botToken = token;
    if (!botToken) { try { const credentials = await this.credentials.get(userId, 'telegram', environment); botToken = typeof credentials.botToken === 'string' ? credentials.botToken : undefined; chatId = chatId ?? (typeof credentials.chatId === 'string' ? credentials.chatId : undefined); } catch { return; } }
    ...
    const poll = async () => {
      try {
        ...
        if (result.ok) for (const update of result.result ?? []) {
          this.offsets.set(key, Number(update.update_id) + 1); const message = update.message; if (!message?.text) continue;
          if (chatId && String(message.chat?.id) !== chatId) continue;
          try { const signal = this.parseSignal(message.text); const accepted = await this.signals.accept(userId, environment, { ...signal, rawMessage: message.text, telegramUpdateId: Number(update.update_id), telegramChatId: String(message.chat?.id ?? '') }); await this.send(botToken!, String(message.chat.id), `TCE ${signal.symbol} ${signal.side} accepted\nENTRY ${signal.entry}\nTP ${signal.tp}\nSL ${signal.sl}\nStatus: ${accepted.status}`); }
          catch (error) { await this.send(botToken!, String(message.chat.id), `TCE signal rejected: ${error instanceof Error ? error.message : 'Invalid signal'}`); }
        }
  ...
  async stop(userId: string, environment = 'production') { const key = `${userId}:${environment}`; this.running.delete(key); const timer = this.timers.get(key); if (timer) clearTimeout(timer); this.timers.delete(key); }
