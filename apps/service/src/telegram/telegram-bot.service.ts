...
export class TelegramBotService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    for (const row of data ?? [])
      await this.start(
        ...
        String(row.credential_name ?? 'default'),
        String(row.id)
      );
  }
  ...
  async testToken(token: string) {
    ...
    const me = await this.telegram(token.trim(), 'getMe');
  ...
  async configure(
    ...
    const verified = await this.testToken(token);
    const saved = await this.credentials.save(
      userId,
      'telegram',
      environment,
      { botToken: token.trim(), chatId: chatId?.trim() || undefined },
      credentialName
    );
  ...
  async start(
    ...
    if (!botToken) {
      try {
        ...
        botToken = typeof stored.botToken === 'string' ? stored.botToken : undefined;
        chatId = chatId ?? (typeof stored.chatId === 'string' ? stored.chatId : undefined);
      } catch {
        return;
      }
    }
    ...
    const poll = async () => {
      try {
        ...
        if (result.ok)
          for (const update of result.result ?? []) {
            ...
            try {
              ...
              await this.debug(userId, 'telegram', 'WARN', `Signal rejected: ${reason}`, {
                environment,
                bot: name,
              });
            }
        ...
        await this.debug(userId, 'telegram', 'ERROR', `Telegram polling failed: ${reason}`, {
          environment,
          bot: name,
        });
      } finally {
  ...
  stopPolling(credentialId: string) {
    this.running.delete(credentialId);
    const timer = this.timers.get(credentialId);
    if (timer) clearTimeout(timer);
    this.timers.delete(credentialId);
    this.offsets.delete(credentialId);
  }
  async stop(userId: string, environment = 'production') {
    await this.removeBot(userId, environment, 'default');
  }
  async listAssignments(userId: string) {
    const { data, error } = await this.supabase.db
      .from('telegram_debug_assignments')
      .select('id,telegram_credential_id,service_name,min_level,enabled')
      .eq('user_id', userId)
      .order('service_name');
    if (error) throw error;
    return data ?? [];
  ...
  async assignDebug(
    ...
    input: {
      ...
      serviceName: string;
      minLevel?: TelegramDebugLevel;
      enabled?: boolean;
    }
  ) {
    const serviceName = input.serviceName.trim();
    if (!serviceName) throw new Error('Service name is required');
    const { data: bot, error: botError } = await this.supabase.db
      .from('platform_credentials')
      .select('id')
      .eq('id', input.telegramCredentialId)
      .eq('user_id', userId)
      .eq('provider', 'telegram')
      .eq('is_active', true)
      .eq('telegram_paused', false)
    ...
    const { data, error } = await this.supabase.db
      ...
      .upsert(
        {
          user_id: userId,
          telegram_credential_id: input.telegramCredentialId,
          service_name: serviceName,
          min_level: input.minLevel ?? 'INFO',
          enabled: input.enabled ?? true,
        },
        { onConflict: 'user_id,telegram_credential_id,service_name' }
      )
      .select('id,telegram_credential_id,service_name,min_level,enabled')
      .single();
    if (error) throw error;
  ...
  async debug(
    ...
    try {
      const { data: assignments, error } = await this.supabase.db
        .from('telegram_debug_assignments')
        .select('telegram_credential_id,service_name,min_level')
        .eq('user_id', userId)
        .eq('enabled', true)
      ...
      for (const row of assignments) {
        ...
        if (LEVEL_WEIGHT[level] >= LEVEL_WEIGHT[minLevel])
          selected.set(String(row.telegram_credential_id), true);
      }
      for (const credentialId of selected.keys()) {
        ...
        const botToken = typeof stored.botToken === 'string' ? stored.botToken : '';
        ...
        await this.send(botToken, chatId, `[${level}] ${serviceName}\n${message}${suffix}`);
      }
    } catch (error) {
      this.logger.debug(
  ...
  async sendToCredential(userId: string, credentialId: string, text: string) {
    const { data: row, error } = await this.supabase.db
      .from('platform_credentials')
      .select('environment,credential_name,is_active,telegram_paused')
      .eq('id', credentialId)
      .eq('user_id', userId)
      .eq('provider', 'telegram')
      .maybeSingle();
    if (error) throw error;
    if (!row || !row.is_active || row.telegram_paused)
      throw new Error('Telegram bot is unavailable');
    const stored = await this.credentials.get(
      userId,
      'telegram',
      String(row.environment ?? 'production'),
  ...
  private async send(token: string, chatId: string, text: string) {
    return this.telegram(token, 'sendMessage', { chat_id: chatId, text });
  }
