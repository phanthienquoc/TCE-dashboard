...
export class TceEngineService {
  ...
  async run(accountId: string, environment = 'production', execute = false) {
    ...
    try {
      ...
      const enrichedPositions = positions.map((position) => {
        ...
        const marketValue = marketPrice * Number(position.quantity ?? 0);
        const costBasis = Number(position.cost_basis ?? (Number(position.avg_cost ?? 0) * Number(position.quantity ?? 0)));
        const unrealizedPnl = marketValue - costBasis;
        const unrealizedPnlPct = costBasis > 0 ? (unrealizedPnl / costBasis) * 100 : undefined;
        return {
      ...
      if (execute) {
        return await this.execute(accountId, environment, state, decisions);
      }
  ...
  private async execute(accountId: string, environment: string, state: TceEngineAccountState, decisions: TceEngineDecision[]) {
    ...
    // Do not place buys in the same pass as pending sells. Wait for SELL fills and re-run
    // so the second decision is based on actual cash/equity from SSI instead of optimistic state.
    for (const decision of sells) {
      const result = await this.ssi.sellAll(accountId, environment, decision.symbol, decision.quantity);
    ...
    if (sells.length > 0) {
      return { skipped: false, decisions, executed, followUpRequired: true, reason: 'sell_orders_submitted_wait_for_fill_before_buy' };
    }

    for (const decision of buys) {
      const result = await this.ssi.buy(accountId, environment, decision.symbol, decision.quantity, state.candidates.find((candidate) => candidate.symbol.toUpperCase() === decision.symbol.toUpperCase())?.price);
  ...
  private async loadConfig(accountId: string): Promise<TceEngineConfig> {
    ...
    return {
      ...
      maxAssetAllocationPct: Number(data?.max_asset_allocation_pct ?? DEFAULT_CONFIG.maxAssetAllocationPct),
  ...
  private async loadAccount(accountId: string) {
    const { data, error } = await this.supabase.db.from('tce_accounts').select('id,capital_available').eq('id', accountId).maybeSingle();
    if (error) throw error;
    return data;
  }

  private async loadPositions(accountId: string) {
    const { data, error } = await this.supabase.db.from('tce_positions').select('symbol,quantity,avg_cost,cost_basis,market_price,market_value,unrealized_pnl,status').eq('account_id', accountId).neq('status', 'CLOSED').order('symbol');
    if (error) throw error;
    return (data ?? []) as Array<Record<string, any>>;
  }

  private async loadCandidates(accountId: string, _environment: string): Promise<TceEngineCandidate[]> {
    const { data, error } = await this.supabase.db.from('tce_buy_candidates').select('symbol,rank,target_price').eq('account_id', accountId).order('rank');
    if (error) throw error;
    if (data?.length) return data.map((row) => ({ symbol: String(row.symbol).toUpperCase(), rank: Number(row.rank ?? Number.MAX_SAFE_INTEGER), price: row.target_price == null ? undefined : Number(row.target_price) }));

    const shared = await this.supabase.db.from('tce_shared_pools').select('symbol,rank').eq('status', 'ACTIVE').order('rank');
    if (shared.error) throw shared.error;
    return (shared.data ?? []).map((row) => ({ symbol: String(row.symbol).toUpperCase(), rank: Number(row.rank ?? Number.MAX_SAFE_INTEGER) }));
  }

  private async loadMarketPrices(accountId: string, symbols: string[]) {
    const { data, error } = await this.supabase.db.from('tce_market_prices').select('symbol,price,observed_at').eq('user_id', accountId).in('symbol', symbols).order('observed_at', { ascending: false });
    if (error) throw error;
    const prices: Record<string, number> = {};
    for (const row of data ?? []) if (prices[row.symbol] == null) prices[row.symbol] = Number(row.price);
    return prices;
  }
}
