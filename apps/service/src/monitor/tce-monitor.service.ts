...
export class TceMonitorService implements OnModuleInit, OnModuleDestroy {
  ...
  async run(): Promise<{
    ...
    if (this.running)
      return { skipped: true, reason: 'already_running', monitored: 0, signals: [] };
    this.running = true;
    ...
    try {
      const { data: configs, error: configError } = await this.supabase.db
        ...
        .select(
          'account_id,max_positions,pool_size,monitor_interval_minutes,market_open,market_close,timezone'
        );
      ...
      for (const config of configs ?? []) {
        ...
        for (const position of (positions ?? []) as Position[]) {
          ...
          if (snapshot.signal === 'CASHOUT' && inserted?.id) {
            const { error: cashoutError } = await this.supabase.db
        ...
        if (active >= (config.max_positions ?? 2)) {
          await this.audit(config.account_id, 'POSITION_MONITOR', started, {
            ...
            skipped: false,
            metadata: {
              pool_scan: 'blocked_by_position_capacity',
          ...
          await this.audit(config.account_id, 'POOL_SCAN', started, {
            ...
            metadata: {
              open_slots: (config.max_positions ?? 2) - active,
      ...
      this.logger.log(
        `TCE monitor complete: ${result.monitored} positions; signals=${result.signals.join(',') || 'none'}`
      );
      return result;
    } catch (error) {
  ...
  private buildSnapshot(position: Position, marketHours: boolean) {
    ...
    return {
      ...
      signal_reason: {
        pnl_pct: pnlPct == null ? null : Number(pnlPct.toFixed(2)),
        thresholds: { take_profit: 5, cashout: 8, cut: -5 },
        price_source: 'tce_positions.market_price',
      },
      t_plus: 2,
