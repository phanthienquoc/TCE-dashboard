...
export class TceSignalService {
  ...
  async accept(accountId: string, environment: string, signal: TceSignalInput) {
    const { data: existing } = signal.telegramUpdateId == null ? { data: null as any } : await this.supabase.db.from('tce_telegram_signals').select('id,status').eq('account_id', accountId).eq('environment', environment).eq('telegram_update_id', signal.telegramUpdateId).maybeSingle();
    if (existing) return { status: existing.status, signalId: existing.id, duplicate: true };
    const { data, error } = await this.supabase.db.from('tce_telegram_signals').insert({ account_id: accountId, environment, telegram_update_id: signal.telegramUpdateId ?? null, telegram_chat_id: signal.telegramChatId ?? null, raw_message: signal.rawMessage ?? '', symbol: signal.symbol, side: signal.side, entry: signal.entry, tp: signal.tp, sl: signal.sl, status: 'QUEUED' }).select('id,status,symbol,side,entry,tp,sl').single();
