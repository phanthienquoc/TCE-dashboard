...
export class ProfitExitSettingsController {
  ...
  async get(@Headers('authorization') auth?: string) {
    ...
    const { data, error } = await this.supabase.db
      ...
      .select(
        'auto_sell_enabled,auto_sell_profit_target_pct,auto_sell_interval_minutes,auto_sell_last_run_at'
      )
  ...
  async set(
    ...
    @Body() body?: { enabled?: boolean; profitTargetPct?: number; intervalMinutes?: number }
  ) {
    if (!auth?.startsWith('Bearer ')) throw new UnauthorizedException('Bearer token required');
    const userId = this.jwt.verify(auth.slice(7)).sub;
    const { data: account, error: accountError } = await this.supabase.db
