...
export class DashboardCapitalPoolEngine {
  calculate(input: {
    ...
    for (const position of input.positions) {
      const pool = String(position.pool ?? '')
        .trim()
        .toUpperCase() as CapitalPoolId;
      if (!pools.includes(pool)) continue;
      const costBasis = Number(position.costBasis ?? position.cost_basis);
      const fallback =
        Number(position.avgBuyCost ?? position.avg_cost ?? 0) * Number(position.quantity ?? 0);
      used.set(
        pool,
        (used.get(pool) ?? 0) +
          (Number.isFinite(costBasis) ? Math.max(0, costBasis) : Math.max(0, fallback))
      );
    ...
    let totalFree = 0;
    for (const pool of pools) {
      const value = Math.max(0, allocated - Math.min(allocated, used.get(pool) ?? 0));
      free.set(pool, value);
      totalFree += value;
    }
    return pools.map(pool => {
      ...
      const pendingT2Capital = Math.min(
        Math.max(0, freeCapacity - availableCapital),
        pendingCash * share
      );
      ...
      return {
        ...
        pendingT2Capital: Math.round(pendingT2Capital),
        usedCapital: Math.round(usedCapital),
        occupiedSlots,
        totalSlots: slots,
        usedRatio: allocated > 0 ? Math.min(1, usedCapital / allocated) : 0,
        state,
      };
    });
  }
