...
export class EngineRuntimeService {
  ...
  async getRuntime(
    accountId: string
  ): Promise<{ accountId: string; engines: EngineRuntimeNode[] }> {
    const [{ data: configs, error: configError }, { data: states, error: stateError }] =
      await Promise.all([
        this.db.db
  ...
  async getStartPlan(accountId: string) {
    ...
    const active = new Set(
      runtime.engines.filter(engine => engine.status === 'ACTIVE').map(engine => engine.engineId)
    );
    ...
    const visit = (engineId: string) => {
      ...
      if (visiting.has(engineId))
        throw new Error(`Engine dependency cycle detected at ${engineId}`);
    ...
    return {
      accountId,
      startOrder: ordered,
      startable: runtime.engines
        .filter(engine => engine.status === 'ACTIVE')
        .map(engine => engine.engineId),
      blocked: runtime.engines
        .filter(engine => engine.status !== 'ACTIVE')
        .map(engine => ({
          engineId: engine.engineId,
          status: engine.status,
          blockedBy: engine.blockedBy,
          error: engine.error,
        })),
    };
  }
