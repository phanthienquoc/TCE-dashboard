...
export class DashboardService {
  ...
  private async getPools(accountId: string, status?: string) {
    ...
    const { data: prices, error: priceError } = await this.supabase.db.from('tce_market_prices').select('symbol,price,close_price,trading_date,observed_at').eq('account_id', accountId).in('symbol', symbols).order('trading_date', { ascending: false }).order('observed_at', { ascending: false });
