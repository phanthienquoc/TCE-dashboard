...
const ENGINE_IDS = ['tce-decision', 'capital-rotation-decision', 'ssi-execution', 'binance-market', 'binance-execution', 'binance-derivatives', 'binance-xau'] as const;
...
export class DashboardService {
  private readonly capitalPoolEngine = new DashboardCapitalPoolEngine();
  ...
  async getNextPositionsForUser(userId: string) {
    ...
    const rows = data ?? [];
    const poolIds = rows.map(candidate => candidate.pool_entry_id).filter((id): id is string => Boolean(id));
    const holdDaysByPoolId = new Map<string, number | null>();
    if (poolIds.length) {
      const { data: poolEntries, error: poolError } = await this.supabase.db.from('tce_pool_entries').select('id,expected_hold_days').in('id', poolIds);
      if (poolError) throw this.dbError('getNextPositionsForUser.poolHoldDays', poolError);
      for (const pool of poolEntries ?? []) holdDaysByPoolId.set(String(pool.id), pool.expected_hold_days == null ? null : Number(pool.expected_hold_days));
    }
    return rows.map(candidate => ({ ...candidate, targetPosition: candidate.target_position == null ? null : Number(candidate.target_position), targetQuantity: candidate.target_quantity == null ? null : Number(candidate.target_quantity), targetPrice: candidate.target_price == null ? null : Number(candidate.target_price), score: candidate.score == null ? null : Number(candidate.score), expectedHoldDays: candidate.pool_entry_id == null ? null : (holdDaysByPoolId.get(String(candidate.pool_entry_id)) ?? null) }));
  }
  ...
  async get(userId: string, poolStatus?: string): Promise<DashboardSnapshot> {
    ...
    const deployed = Number(account.capital_deployed ?? 0); const cash = Number(account.capital_available ?? 0); const marketValue = positions.reduce((sum, p) => sum + Number(p.market_value ?? 0), 0); const unrealized = positions.reduce((sum, p) => sum + Number(p.unrealized_pnl ?? 0), 0); const totalValue = Math.round(cash + marketValue);
    const capitalPools = this.capitalPoolEngine.calculate({ totalCapital: totalValue, availableCash: cash, positions });
    const brokerAccounts = sources.find(source => source.source === 'ssi')?.data as Record<string, unknown> | undefined;
    return { account: { userId, accountId: account.id, name: account.name, initial_capital: Number(account.initial_capital ?? 0), capital_deployed: Math.round(deployed), capital_available: cash, cashout_target: Number(account.cashout_target ?? 0), cashout_realized: Number(account.cashout_realized ?? 0), recovery_remaining: Number(account.recovery_remaining ?? 0), current_cycle: Number(account.current_cycle ?? 1), unrealized_pnl: Math.round(unrealized), market_value: Math.round(marketValue), totalValue, capitalPools, max_positions: 0, pool_size: pools.length }, positions, orders, pools, nextPositions, balance: { cash, equity: cash + marketValue, withdrawable: cash, source: 'supabase' }, brokerAccounts: brokerAccounts?.configured ? [{ provider: 'ssi', accountNo: brokerAccounts.accountNo ?? undefined, environment: 'production', status: 'Connected' }] : [], sources };
  }
  private async resolveAccount(userId: string) {
    ...
    const user = await this.supabase.db.from('users').select('id,email').eq('id', userId).maybeSingle(); if (user.error) throw this.dbError('resolveAccount.user', user.error); if (!user.data?.email) throw new NotFoundException('Authenticated user is not configured');
    const candidate = await this.supabase.db.from('tce_accounts').select('id,user_id,name,initial_capital,cashout_target,cashout_realized,capital_deployed,capital_available,recovery_remaining,current_cycle,status').eq('name', `USER:${user.data.email}`).maybeSingle(); if (candidate.error) throw this.dbError('resolveAccount.candidate', candidate.error); if (!candidate.data) throw new NotFoundException('TCE account is not configured for this user');
    const repaired = await this.supabase.db.from('tce_accounts').update({ user_id: userId, updated_at: new Date().toISOString() }).eq('id', candidate.data.id).select('id,user_id,name,initial_capital,cashout_target,cashout_realized,capital_deployed,capital_available,recovery_remaining,current_cycle,status').single(); if (repaired.error) throw this.dbError('resolveAccount.repair', repaired.error); return repaired.data;
  }
  private async getPools(accountId: string, userId: string, status?: string) {
    ...
    const normalizedStatus = String(status ?? '').trim().toUpperCase(); if (normalizedStatus) query = query.eq('status', normalizedStatus);
    const { data, error } = await query.order('rank', { ascending: true }).limit(50); if (error) throw this.dbError('getPools', error);
    const rows = data ?? []; const symbols = [...new Set(rows.map(row => String(row.symbol).toUpperCase()).filter(Boolean))]; const marketPriceBySymbol = new Map<string, number>();
    if (symbols.length) { const { data: marketRows, error: marketError } = await this.supabase.db.from('tce_market_prices').select('symbol,price,trading_date,observed_at').eq('user_id', userId).in('symbol', symbols).order('trading_date', { ascending: false }).order('observed_at', { ascending: false }); if (marketError) throw this.dbError('getPools.marketPrices', marketError); for (const row of marketRows ?? []) { const symbol = String(row.symbol ?? '').toUpperCase(); if (symbol && !marketPriceBySymbol.has(symbol)) { const value = Number(row.price); if (Number.isFinite(value)) marketPriceBySymbol.set(symbol, value); } } }
