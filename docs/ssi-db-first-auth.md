...
The application-layer session cache is not used as the credential source for normal API requests, preventing stale in-memory tokens from causing SSI 401 responses after another request or process has rotated the token.
