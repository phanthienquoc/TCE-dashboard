...
create table if not exists public.tce_pool_entries (
  id uuid primary key default gen_random_uuid(), account_id uuid not null references public.tce_accounts(id) on delete cascade,
  symbol text not null, rank integer not null check (rank > 0),
  status text not null default 'WATCHING' check (status in ('WATCHING','TRIGGERED','POSITIONED','REJECTED','EXPIRED','REMOVED')),
  score numeric(8,3), cashout_score numeric(8,3), liquidity_score numeric(8,3), catalyst_score numeric(8,3), recovery_score numeric(8,3), risk_score numeric(8,3),
  entry_low numeric(18,4), entry_high numeric(18,4), target_price numeric(18,4), invalidation_price numeric(18,4), expected_cashout bigint,
  expected_return_pct numeric(10,4), expected_hold_days integer, rationale jsonb not null default '{}'::jsonb,
...
create table if not exists public.tce_position_snapshots (
  id uuid primary key default gen_random_uuid(), account_id uuid not null references public.tce_accounts(id) on delete cascade,
  symbol text not null, observed_at timestamptz not null default now(), market_price numeric(18,4), quantity integer not null default 0 check (quantity >= 0), avg_cost numeric(18,4),
  market_value bigint, cost_basis bigint, unrealized_pnl bigint, unrealized_pnl_pct numeric(10,4), volume bigint, turnover bigint,
  signal text not null default 'HOLD' check (signal in ('HOLD','WATCH','TAKE_PROFIT','CUT','CASHOUT','EXIT')),
  signal_score numeric(8,3), signal_reason jsonb not null default '{}'::jsonb, t_plus integer not null default 2 check (t_plus between 0 and 3),
  is_market_hours boolean not null default false, created_at timestamptz not null default now()
);
...
create table if not exists public.tce_monitor_runs (
  id uuid primary key default gen_random_uuid(), account_id uuid not null references public.tce_accounts(id) on delete cascade,
  run_type text not null check (run_type in ('POSITION_MONITOR','POOL_SCAN','FULL_SCAN')), started_at timestamptz not null default now(), finished_at timestamptz,
  market_session boolean not null default false, active_position_count integer not null default 0, pool_count integer not null default 0, positions_monitored integer not null default 0,
  signals_found integer not null default 0, skipped boolean not null default false, skip_reason text, error_message text, metadata jsonb not null default '{}'::jsonb
);
...
create table if not exists public.tce_cashout_events (
  id uuid primary key default gen_random_uuid(), account_id uuid not null references public.tce_accounts(id) on delete cascade,
  symbol text not null, position_snapshot_id uuid references public.tce_position_snapshots(id) on delete set null,
  event_type text not null check (event_type in ('DIVIDEND_RIGHT','DIVIDEND_CASH','PRICE_CASHOUT','PARTIAL_EXIT','FULL_EXIT','RECYCLE')),
  event_date date, ex_right_date date, payment_date date, gross_cash bigint not null default 0, tax bigint not null default 0, net_cash bigint not null default 0,
  capital_released bigint not null default 0, realized_pnl bigint not null default 0, notes jsonb not null default '{}'::jsonb, created_at timestamptz not null default now()
);
...
select a.id account_id, coalesce(c.max_positions,2) max_positions, count(p.symbol)::integer active_positions,
  greatest(coalesce(c.max_positions,2)-count(p.symbol)::integer,0) open_slots
from public.tce_accounts a left join public.tce_strategy_config c on c.account_id=a.id left join public.tce_positions p on p.account_id=a.id and p.status is distinct from 'CLOSED'
...
create policy if not exists tce_strategy_config_read on public.tce_strategy_config for select using (true);
create policy if not exists tce_pool_entries_read on public.tce_pool_entries for select using (true);
create policy if not exists tce_position_snapshots_read on public.tce_position_snapshots for select using (true);
create policy if not exists tce_monitor_runs_read on public.tce_monitor_runs for select using (true);
create policy if not exists tce_cashout_events_read on public.tce_cashout_events for select using (true);
