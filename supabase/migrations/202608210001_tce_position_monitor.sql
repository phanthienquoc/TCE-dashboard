...
-- Additive migration for the existing TCE Supabase schema.
...
create table if not exists public.tce_pool_entries (
  ...
  score numeric(8,3), cashout_score numeric(8,3), liquidity_score numeric(8,3),
  catalyst_score numeric(8,3), recovery_score numeric(8,3), risk_score numeric(8,3),
  entry_low numeric(18,4), entry_high numeric(18,4), target_price numeric(18,4),
  invalidation_price numeric(18,4), expected_cashout bigint,
  expected_return_pct numeric(10,4), expected_hold_days integer,
  rationale jsonb not null default '{}'::jsonb,
  observed_at timestamptz not null default now(), expires_at timestamptz,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
  unique(account_id, symbol, observed_at)
);
create index if not exists idx_tce_pool_active on public.tce_pool_entries(account_id, status, rank)
...
create table if not exists public.tce_position_snapshots (
  ...
  market_price numeric(18,4), quantity integer not null default 0 check (quantity >= 0),
  avg_cost numeric(18,4), market_value bigint, cost_basis bigint, unrealized_pnl bigint,
  unrealized_pnl_pct numeric(10,4), volume bigint, turnover bigint,
  signal text not null default 'HOLD' check (signal in ('HOLD','WATCH','TAKE_PROFIT','CUT','CASHOUT','EXIT')),
  signal_score numeric(8,3), signal_reason jsonb not null default '{}'::jsonb,
...
create table if not exists public.tce_monitor_runs (
  ...
  market_session boolean not null default false, active_position_count integer not null default 0,
  pool_count integer not null default 0, positions_monitored integer not null default 0,
  signals_found integer not null default 0, skipped boolean not null default false,
  skip_reason text, error_message text, metadata jsonb not null default '{}'::jsonb
);
...
create table if not exists public.tce_cashout_events (
  ...
  event_date date, ex_right_date date, payment_date date,
  gross_cash bigint not null default 0, tax bigint not null default 0, net_cash bigint not null default 0,
  capital_released bigint not null default 0, realized_pnl bigint not null default 0,
  notes jsonb not null default '{}'::jsonb, created_at timestamptz not null default now()
);
...
create trigger trg_tce_strategy_config_updated_at before update on public.tce_strategy_config
...
create trigger trg_tce_pool_entries_updated_at before update on public.tce_pool_entries
for each row execute function public.tce_touch_updated_at();

create or replace view public.tce_active_pool
...
) ranked where pool_position <= 5;

create or replace view public.tce_position_capacity
with (security_invoker=true) as
select a.id as account_id, coalesce(c.max_positions, 2) as max_positions,
...
where p.status is distinct from 'CLOSED'
group by a.id, c.max_positions;

-- RLS: dashboard can read; writes should use the server-side worker/service role.
...
comment on table public.tce_pool_entries is 'Rolling TCE hunting pool; default top 5 candidates. Refreshed only when position capacity exists.';
