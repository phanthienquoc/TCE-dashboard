...
left join lateral (
  select
    min(mp.price) as one_year_low,
    max(mp.price) as one_year_high
  from public.tce_market_prices mp
  where upper(mp.symbol) = upper(se.symbol)
    and mp.trading_date >= (current_date - interval '365 days')::date
    and mp.trading_date <= current_date
) history on true;

comment on view public.tce_stock_event_market_metrics is
  'Upcoming stock-event market metrics: latest price, dividend yield, and 365-day low/high. Historical values accumulate from the stock-event cron price sync.';
