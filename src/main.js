...
function positionsView(d){
  ...
  return `<section class="section page-section"><div class="section-head"><div><h2>Positions</h2><small>Open positions</small></div><span>${d.positions.length}</span></div><div class="cards">${d.positions.length?d.positions.map(p=>`<div class="position"><div><b>${p.symbol}</b><small>${p.quantity} cp • avg ${num(p.avg_cost)}</small></div><div class="right"><b>${money(p.market_value||p.cost_basis)}</b><small class="${(p.unrealized_pnl||0)>=0?'up':'down'}">${(p.unrealized_pnl||0)>=0?'+':''}${money(p.unrealized_pnl||0)}</small></div></div>`).join(''):'<div class="empty">No open positions</div>'}</div></section><section class="section planned"><div class="section-head"><div><h2>Next 5 positions</h2><small>Planned buying candidates</small></div><span>${candidates.length}/5</span></div><div class="planned-list">${Array.from({length:5},(_,i)=>candidates[i]).map((c,i)=>c?`<div class="planned-row"><span class="slot">${c.rank}</span><div><b>${c.symbol}</b><small>${c.target_quantity?`${num(c.target_quantity)} cp`:''}${c.target_price?` • target ${num(c.target_price)}`:''}${c.reason?` • ${c.reason}`:''}</small></div><span class="planned-status ${c.status==='ready'?'ready':''}">${c.status}</span></div>`:`<div class="planned-row"><span class="slot">${i+1}</span><div><b>Position ${i+1}</b><small>Waiting for next buy candidate</small></div><span class="planned-status">Available</span></div>`).join('')}</div></section>`;
}
...
async function render(d,path=route()){
  ...
  document.querySelector('#app').innerHTML=`<main><header><div><span class="eyebrow">TCE • TREASURY CASH EXTRACTION</span><h1>${path==='/positions'?'Positions':path==='/orders'?'Recent orders':path==='/platforms'?'Trading Platforms':'Dashboard'}</h1></div><span class="live">● MANUAL</span></header>${page}<nav>${[['⌂','Overview','/'],['◫','Positions','/positions'],['↕','Orders','/orders'],['⚙','Platforms','/platforms']].map(([icon,label,to],i)=>`<button class="${i===active?'active':''}" data-route="${to}">${icon}<small>${label}</small></button>`).join('')}</nav></main>`;
