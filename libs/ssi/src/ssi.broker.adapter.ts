...
export class SsiBrokerAdapter implements BrokerPort, SsiConnectionPort {
  ...
  async connect(_input: ConnectInput) {
    return this.result(async () => {
  ...
  async health(input: ConnectInput): Promise<ContractResult<PlatformHealth>> {
    ...
    return this.result(async () => {
      if (!this.auth?.getToken()) await this.connect(input);
      return {
        provider: 'ssi',
        available: true,
  ...
  async test(input: SsiAuthInput): Promise<ContractResult<SsiConnectionTest>> {
    return this.result(async () => {
      ...
      } as SsiConnectionTest;
    });
  }

  private trading() {
    if (!this.tradingClient) throw new Error('SSI is not connected');
  ...
  async balance(accountNo: string) {
    return this.result(async () => {
      ...
      return {
        // SDK 3.2.0 exposes accountBalance as the equity-account cash balance.
        cash: Number(balance?.accountBalance ?? 0),
        equity: Number(balance?.accountBalance ?? 0),
        withdrawable: Number(balance?.withdrawable ?? 0),
        source: 'ssi',
      } as AccountBalance;
    });
  }
    ...
    return this.result(async () => {
      ...
      return (orders ?? [])
        ...
        .map((order) => ({
          ...
          quantity: Number(order.filledQuantity ?? order.quantity ?? 0),
          price: Number(order.avgPrice ?? order.price ?? 0),
