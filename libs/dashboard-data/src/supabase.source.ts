...
export class SupabaseDashboardSource implements DashboardSourcePort {
  ...
  async snapshot(userId: string): Promise<ContractResult<unknown>> {
    try {
      ...
      const [positions, orders, pools, nextPositions] = await Promise.all([
        ...
        this.db.from('tce_orders').select('*').eq('account_id', account.id).order('created_at', { ascending: false }),
        this.db.from('tce_pool_entries').select('*').eq('account_id', account.id).order('rank', { ascending: true }),
        this.db.from('tce_buy_candidates').select('*').eq('account_id', account.id).in('status', ['queued', 'ready']).order('rank', { ascending: true })
      ]);
  ...
  private providerError(message: string, retryable = true): ContractResult<unknown> {
    return { ok: false, error: { code: 'PROVIDER_ERROR', message, retryable, provider: this.source } };
  }
}
