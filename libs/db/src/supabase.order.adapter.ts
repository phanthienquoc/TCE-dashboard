...
export class SupabaseOrderAdapter implements OrderRepository {
  ...
  async list(accountId: string) {
    ...
    return (data ?? []).map(o => ({
      accountId,
      externalId: String(o.note ?? o.id),
      symbol: String(o.symbol),
      side: String(o.side).toUpperCase() === 'SELL' ? ('SELL' as const) : ('BUY' as const),
      quantity: Number(o.quantity ?? 0),
      price: o.price == null ? undefined : Number(o.price),
  ...
  async upsert(order: AccountOrder) {
    ...
    const row = {
      ...
      gross_value: gross,
      fee_tax: 0,
      net_cashflow: order.side === 'BUY' ? -gross : gross,
      cycle_no: 0,
      status: order.status,
      note,
    };
    ...
    const result = existing.data
      ? await this.db.from('tce_orders').update(row).eq('id', existing.data.id)
      : await this.db.from('tce_orders').insert(row);
