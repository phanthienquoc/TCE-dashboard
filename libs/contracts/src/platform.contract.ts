...
export type AccountPosition = { symbol: string; quantity: number; averagePrice: number; marketPrice?: number; marketValue?: number; unrealizedPnl?: number; source: PlatformKind };
export type AccountOrder = { externalId: string; symbol: string; side: 'BUY'|'SELL'; quantity: number; price?: number; status: string; createdAt?: string; source: PlatformKind };
