...
export class TceEngine implements TceEnginePort {
  evaluate(state: TceEngineAccountState, config: TceEngineConfig): TceEngineDecision[] {
    ...
    for (const position of state.positions) {
      ...
      const profitPct = Number(
        position.unrealizedPnlPct ??
          (costBasis > 0 ? ((marketValue - costBasis) / costBasis) * 100 : NaN)
      );
    ...
    const sellSymbols = new Set(
      decisions.filter(d => d.action === 'SELL').map(d => d.symbol.toUpperCase())
    );
    const openSymbols = new Set(
      state.positions
        .filter(p => Number(p.quantity || 0) > 0 && !sellSymbols.has(p.symbol.toUpperCase()))
        .map(p => p.symbol.toUpperCase())
    );
    const projectedAssetCount = openSymbols.size;
    if (projectedAssetCount >= normalized.maxTotalAssets || availableBudget <= 0)
        ...
        : [
            {
              action: 'HOLD',
              reason:
                projectedAssetCount >= normalized.maxTotalAssets
                  ? 'max_assets_reached'
                  : 'no_available_budget',
            },
          ];
    ...
    if (!candidate)
      return decisions.length ? decisions : [{ action: 'HOLD', reason: 'no_pool_candidate' }];

    const price = Number(candidate.price ?? 0);
    if (!Number.isFinite(price) || price <= 0)
      return decisions.length
        ? decisions
        : [{ action: 'HOLD', reason: 'candidate_price_unavailable' }];

    const existingValue = Number(
      state.positions.find(p => p.symbol.toUpperCase() === candidate.symbol.toUpperCase())
        ?.marketValue ?? 0
    );
    ...
    const quantity =
      Math.floor(budgetForBuy / price / normalized.buyQuantityStep) * normalized.buyQuantityStep;
    const estimatedValue = quantity * price;

    if (quantity < normalized.buyQuantityStep || estimatedValue <= 0)
      return decisions.length
        ? decisions
        : [{ action: 'HOLD', reason: 'budget_below_minimum_lot_or_allocation_capacity' }];
    ...
    decisions.push({
      ...
      reason: 'reinvest_remaining_budget_with_risk_cap',
    });
    return decisions;
  }
  ...
  private normalizeConfig(config: TceEngineConfig): TceEngineConfig {
    return {
      ...
      profitTargetPct: Math.max(
        0,
        finitePositive(config.profitTargetPct, DEFAULT_TCE_ENGINE_CONFIG.profitTargetPct)
      ),
      maxTotalAssets: Math.max(
        1,
        Math.floor(finitePositive(config.maxTotalAssets, DEFAULT_TCE_ENGINE_CONFIG.maxTotalAssets))
      ),
      maxAssetAllocationPct: Math.min(
        100,
        finitePositive(
          config.maxAssetAllocationPct,
          DEFAULT_TCE_ENGINE_CONFIG.maxAssetAllocationPct
        )
      ),
      buyQuantityStep: Math.max(
        1,
        Math.floor(
          finitePositive(config.buyQuantityStep, DEFAULT_TCE_ENGINE_CONFIG.buyQuantityStep)
        )
      ),
