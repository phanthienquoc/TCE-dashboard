  ...
  const decisions = engine.evaluate(
    {
      ...
      positions: [],
      candidates: [{ symbol: 'AAA', rank: 1, price: 31_000 }],
    },
    { ...DEFAULT_TCE_ENGINE_CONFIG, enabled: true }
  );
  assert.deepEqual(decisions, [
...
test('does not add a sixth asset', () => {
  const engine = new TceEngine();
  const positions = ['A', 'B', 'C', 'D', 'E'].map(symbol => ({
    symbol,
    quantity: 100,
    averagePrice: 10,
    marketPrice: 10,
  ...
  const decisions = engine.evaluate(
    {
      accountId: 'a',
      totalAssetsValue: 100_000,
      availableBudget: 50_000,
      positions,
      candidates: [{ symbol: 'F', price: 10, rank: 1 }],
    },
    { ...DEFAULT_TCE_ENGINE_CONFIG, enabled: true }
  );
