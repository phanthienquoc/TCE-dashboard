...
export class HuntingDividendDecisionEngine implements DecisionEngine {
    ...
    const candidates = context.candidates
      ...
      .sort(
        (a, b) =>
          ...
          normalizeSymbol(a.candidate.symbol).localeCompare(normalizeSymbol(b.candidate.symbol)) ||
          a.index - b.index
      );
    ...
    for (const pool of POOLS) {
      ...
      for (let slotIndex = 1; slotIndex <= cfg.slotsPerPool; slotIndex += 1) {
        ...
        while (candidateIndex < candidates.length) {
          ...
          const invalidation =
            cfg.invalidationPercent === undefined
              ? undefined
  ...
  snapshot(decision: TradeDecision): DecisionSnapshot {
    const strategyVersion =
      decision.strategyVersion ?? this.options.strategyVersion ?? HUNTING_DIVIDEND_STRATEGY_VERSION;
    const decisionId =
      decision.decisionId ??
      `${strategyVersion}:${decision.symbol ?? 'UNKNOWN'}:${decision.decision}:${decision.slot ?? 'UNASSIGNED'}:${decision.timestamp}`;
    return { decisionId, strategyVersion, decision: { ...decision, decisionId, strategyVersion } };
  }

  private decideExistingPositions(
    ...
    return context.positions.map(position => {
      ...
      if (entitlement === 'UNKNOWN')
        return basePositionDecision(
          position,
          'WAIT',
          decisionId,
          cfg.strategyVersion,
          entitlement,
          ['dividend_entitlement_unknown'],
          context.timestamp
        );
      if (entitlement === 'AT_RISK' || entitlement === 'NOT_ELIGIBLE')
        return basePositionDecision(
          position,
          'HOLD',
          decisionId,
      ...
      if (profitNet !== undefined && dividendNet !== undefined && profitNet > dividendNet)
        return {
          ...basePositionDecision(
            ...
            cfg.strategyVersion,
            entitlement,
            ['dividend_entitlement_protected', 'profit_net_exceeds_dividend_net'],
            context.timestamp
          ),
          ...common,
        };
...
function resolveConfig(
  ...
  options: HuntingDividendDecisionOptions
): ResolvedConfig {
  const source = { ...(config ?? {}), ...options };
  return {
    lookbackDays: positive(source.lookbackDays, DEFAULTS.lookbackDays),
    tpPercent: positive(source.tpPercent, DEFAULTS.tpPercent),
    ...
    invalidationPercent: optionalPositive(source.invalidationPercent),
    strategyVersion: String(source.strategyVersion ?? HUNTING_DIVIDEND_STRATEGY_VERSION),
  };
}
...
function resolveEvent(
  ...
  const candidate = context.candidates.find(
    c => normalizeSymbol(c.symbol) === normalizeSymbol(position.symbol)
  );
  if (
    !candidate &&
    !position.exRightDate &&
    !position.recordDate &&
    !position.paymentDate &&
    position.dividendNet === undefined &&
    position.dividendGross === undefined
  )
    return undefined;
  return {
    exRightDate: position.exRightDate ?? candidate?.exRightDate,
    recordDate: position.recordDate ?? candidate?.recordDate,
    paymentDate: position.paymentDate ?? candidate?.paymentDate,
    dividendValue: candidate?.dividendValue,
    dividendNet: position.dividendNet ?? candidate?.dividendNet,
  };
}
...
function calculateProfitNet(position: DecisionPositionState): number | undefined {
  if (
...
function calculateDividendNet(
  ...
  if (position.dividendNet !== undefined && Number.isFinite(position.dividendNet))
    return position.dividendNet;
  if (event?.dividendNet !== undefined && Number.isFinite(event.dividendNet))
    return event.dividendNet;
  if (event?.dividendValue !== undefined && Number.isFinite(event.dividendValue))
    return event.dividendValue * position.quantity;
  if (position.dividendGross !== undefined && Number.isFinite(position.dividendGross))
    return position.dividendGross;
  return undefined;
}
function targetReached(position: DecisionPositionState, tpPercent: number): boolean {
  if (position.currentPrice === undefined || !Number.isFinite(position.currentPrice)) return false;
  if (position.targetPrice !== undefined && Number.isFinite(position.targetPrice))
    return position.currentPrice >= position.targetPrice;
  if (position.entryPrice === undefined || !Number.isFinite(position.entryPrice)) return false;
  return position.currentPrice >= position.entryPrice * (1 + tpPercent / 100);
}
function basePositionDecision(
  position: DecisionPositionState,
  action: 'HOLD' | 'SELL' | 'WAIT',
  decisionId: string,
  strategyVersion: string,
  ...
  return {
    ...
    entitlementStatus,
    decisionId,
    decisionWindowKey: decisionId.split(':').slice(-1)[0],
    strategyVersion,
    reasons,
    timestamp,
  };
}
function entitlementAction(status: DividendEntitlementStatus): string {
  return status === 'PROTECTED' || status === 'CONFIRMED' ? 'EXIT_GATE' : 'HOLD_GATE';
}
...
function candidateIdOf(candidate: Record<string, unknown>, index: number): string {
  ...
  return `${normalizeSymbol(String(candidate.symbol))}:${candidate.gdkhqTimestamp ?? 'NO_EVENT_DATE'}:${index}`;
}
