...
test('rejects stale intents', () => {
  const result = evaluateRiskSafetyGate(
    request({ context: { ...context, now: '2026-09-10T01:02:00.000Z' } }),
    config
  );
  assert.equal(result.ok, false);
...
test('rejects stale or missing market data when freshness is required', () => {
  ...
  const stale = evaluateRiskSafetyGate(
    request({ context: { ...context, marketDataAt: '2026-09-10T00:59:00.000Z' } }),
    freshnessConfig
  );
  assert.equal(stale.ok, false);
  if (!stale.ok) assert.equal(stale.code, 'STALE_MARKET_DATA');

  const fresh = evaluateRiskSafetyGate(
    request({ context: { ...context, marketDataAt: context.now } }),
    freshnessConfig
  );
...
test('rejects required dividend data without a freshness policy or with stale data', () => {
  const noPolicy = evaluateRiskSafetyGate(
    request({ context: { ...context, dividendDataRequired: true } }),
    config
  );
  ...
  const stale = evaluateRiskSafetyGate(
    request({
      context: {
        ...
        dividendDataAt: '2026-09-10T00:59:00.000Z',
      },
    }),
    freshConfig
  );
  ...
  const fresh = evaluateRiskSafetyGate(
    request({ context: { ...context, dividendDataRequired: true, dividendDataAt: context.now } }),
    freshConfig
  );
  assert.equal(fresh.ok, true);
});
...
test('rejects major-news risk when configured', () => {
  const result = evaluateRiskSafetyGate(request({ context: { ...context, majorNewsRisk: true } }), {
    ...config,
...
test('rejects global and pool kill switches', () => {
  const globalStop = evaluateRiskSafetyGate(
  ...
  const poolStop = evaluateRiskSafetyGate(
    request({ context: { ...context, killedPools: ['A'] } }),
    config
  );
...
test('blocks LIVE execution while the engine is not RUNNING', () => {
  ...
  const liveRunning = evaluateRiskSafetyGate(request({ intent: liveIntent }), config);
  assert.equal(liveRunning.ok, true);
});

test('rejects blocked and non-allow-listed symbols', () => {
  const blocked = evaluateRiskSafetyGate(
    request({ context: { ...context, blockedSymbols: ['dpm'] } }),
    config
  );
  assert.equal(blocked.ok, false);
  if (!blocked.ok) assert.equal(blocked.code, 'SYMBOL_BLOCKED');

  const notAllowed = evaluateRiskSafetyGate(
    request({ context: { ...context, allowedSymbols: ['HPG'] } }),
    config
  );
...
test('rejects buying power and concurrent exposure violations', () => {
  const capital = evaluateRiskSafetyGate(
    request({ context: { ...context, availableCapital: 1_000_000 } }),
    config
  );
  assert.equal(capital.ok, false);
  if (!capital.ok) assert.equal(capital.code, 'BUYING_POWER_EXCEEDED');

  const exposure = evaluateRiskSafetyGate(
    request({ context: { ...context, concurrentExposure: 19_000_000 } }),
    config
  );
...
test('requires explicit audit fields for manual override and allows an approved risk override', () => {
  const invalidOverride = evaluateRiskSafetyGate(
    request({
      riskAmount: 600_000,
  ...
  const validOverride = evaluateRiskSafetyGate(
    request({
      riskAmount: 600_000,
      override: {
        approved: true,
