...
export function roundPriceToTick(price: number, tick: number): OrderPrecisionOutcome<number> {
  if (!Number.isFinite(price) || price <= 0)
    return { ok: false, code: 'INVALID_PRICE', message: 'Price must be positive and finite' };
  ...
  if (value <= 0)
    return { ok: false, code: 'INVALID_PRICE', message: 'Rounded price must be positive' };
  return { ok: true, value };
}

export function roundQuantityToStep(quantity: number, step: number): OrderPrecisionOutcome<number> {
  if (!Number.isFinite(quantity) || quantity <= 0)
    return { ok: false, code: 'INVALID_QUANTITY', message: 'Quantity must be positive and finite' };
  ...
  if (value <= 0)
    return {
      ...
      message: 'Quantity cannot purchase one quantity step',
    };
  return { ok: true, value };
}
...
export function normalizeOrderPrices(
  ...
  const normalizedTarget =
    target === undefined ? undefined : roundPriceToTick(target, config.priceTick);
  if (normalizedTarget && !normalizedTarget.ok) return normalizedTarget;
  const normalizedInvalidation =
    invalidation === undefined ? undefined : roundPriceToTick(invalidation, config.priceTick);
