...
function allocator() {
  return new CapitalSlotAllocator(createAllocatorSnapshot([
...
test('reserves the selected pool and deterministic slot before producing a plan', () => {
  const a = allocator();
  const result = allocateAndPlanBuyOrder(a, decision, { lotSize: 100 }, 5_000_000, '2026-09-10T00:00:01.000Z');
...
test('does not leak reservation when planning cannot buy one lot', () => {
  const a = allocator();
  const result = allocateAndPlanBuyOrder(a, decision, { lotSize: 100 }, 10_000, '2026-09-10T00:00:01.000Z');
...
test('allocator prevents planner from exceeding the pool reservation', () => {
  const a = allocator();
  const first = allocateAndPlanBuyOrder(a, decision, { lotSize: 100 }, 6_000_000, '2026-09-10T00:00:01.000Z');
  assert.equal(first.ok, true);
  const second = allocateAndPlanBuyOrder(a, { ...decision, id: 'decision-2', candidateId: 'candidate-2', slotId: 'A:2' }, { lotSize: 100 }, 6_000_000, '2026-09-10T00:00:02.000Z');
