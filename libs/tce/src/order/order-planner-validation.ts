...
export type OrderPlannerValidationOutcome =
  Readonly<{ ok: true }> | Readonly<{ ok: false; code: string; message: string }>;
...
export function validateOrderPlannerConfig(
  ...
  if (
    config.quantityStep !== undefined &&
    (!Number.isInteger(config.quantityStep) || config.quantityStep <= 0)
  ) {
    return {
      ok: false,
      code: 'INVALID_QUANTITY_STEP',
      message: 'quantityStep must be a positive integer',
    };
  }
  if (
    config.priceTick !== undefined &&
    (!Number.isFinite(config.priceTick) || config.priceTick <= 0)
  ) {
    return {
      ok: false,
  ...
  if (
    config.minQuantity !== undefined &&
    (!Number.isFinite(config.minQuantity) || config.minQuantity <= 0)
  ) {
    return {
      ok: false,
      code: 'INVALID_MIN_QUANTITY',
      message: 'minQuantity must be positive and finite',
    };
  }
  if (
    config.maxQuantity !== undefined &&
    (!Number.isFinite(config.maxQuantity) || config.maxQuantity <= 0)
  ) {
    return {
      ok: false,
      code: 'INVALID_MAX_QUANTITY',
      message: 'maxQuantity must be positive and finite',
    };
  }
  if (
    config.minQuantity !== undefined &&
  ...
  if (
    config.maxNotional !== undefined &&
    (!Number.isFinite(config.maxNotional) || config.maxNotional <= 0)
  ) {
    return {
      ok: false,
      code: 'INVALID_MAX_NOTIONAL',
      message: 'maxNotional must be positive and finite',
    };
  }
  return { ok: true };
}
...
export function validateBuyOrderPrices(
  ...
  if (target !== undefined && (!Number.isFinite(target) || target <= 0)) {
    return {
      ...
      message: 'Target price must be positive and finite',
    };
  }
  if (invalidation !== undefined && (!Number.isFinite(invalidation) || invalidation <= 0)) {
    return {
      ok: false,
      code: 'INVALID_INVALIDATION_PRICE',
      message: 'Invalidation price must be positive and finite',
    };
  }
