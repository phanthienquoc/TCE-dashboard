...
test('position recovery reconstructs the persisted position after restart', async () => {
...
test('position recovery rebuilds deterministic slot/capital ownership from authoritative positions', () => {
...
test('position recovery ignores invalid positions and duplicate ownership keys', () => {
  assert.equal(
    rebuildPositionOwnership([
      position,
      { ...position, symbol: 'VNM' },
